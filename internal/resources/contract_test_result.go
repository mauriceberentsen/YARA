...
func (r ContractTestResult) Validate() diagnostics.Report {
	...
	if !slices.Contains([]string{"preflight", "runtime-smoke", "model-inference", "capacity-boundary", "sustained-capacity", "policy-contract", "lifecycle-contract"}, r.Spec.Mode) || !slices.Contains([]string{"passed", "failed", "blocked"}, r.Spec.Outcome) {
