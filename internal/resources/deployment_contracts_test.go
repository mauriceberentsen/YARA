...
func validReceipt(t *testing.T) DeploymentReceipt {
	...
	result := DeploymentReceipt{APIVersion: APIVersion, Kind: "DeploymentReceipt", Metadata: DeploymentReceiptMetadata{Name: "receipt"}, Spec: DeploymentReceiptSpec{
		Outcome: "succeeded", StartedAt: now.Format(time.RFC3339Nano), CompletedAt: now.Add(time.Minute).Format(time.RFC3339Nano), ExecutionCorrelationID: "execution-1", PlanID: testDigest('a'), BundleID: testDigest('b'), PreflightResultID: testDigest('c'), ChangeSetID: testDigest('d'), ApprovalID: testDigest('e'), AuthorizationID: testDigest('f'), Target: TargetIdentity{Type: "kubernetes", ReferenceDigest: testDigest('a'), ServerVersion: "v1.35.2"}, Executor: DeploymentExecutorIdentity{Name: "executor", Version: "0.1.0", BinaryDigest: testDigest('a')}, Operations: []DeploymentOperationReceipt{{Resource: KubernetesObjectReference{APIVersion: "v1", Kind: "Namespace", Name: "example"}, Action: "create", Outcome: "applied", AfterDigest: testDigest('b')}}, Postflight: []DeploymentPostflightCheck{{ID: "namespace.exists", Status: "passed", EvidenceDigest: evidence}}, Limitations: []string{"Example."},
	}}
	var err error
