...
func applyKubernetesDeploymentAt(args []string, stdout, stderr io.Writer, now func() time.Time) int {
	...
	bundle, preflight, changeSet, approval, authorization, importReceipt, transferReceipts, scanReceipts, gateResult, code, err := loadAndValidateExecutionInputs(options, now())
	if err != nil {
		return writeLoadErrorWithExit(stdout, code, err, ExitInfeasible)
	}
	correlationID := fmt.Sprintf("deployment-%d", now().UTC().UnixNano())
	subjects := executionSubjects(bundle, preflight, changeSet, approval, authorization, importReceipt, transferReceipts, scanReceipts, gateResult)
	...
	receipt, err := buildDeploymentReceipt(options.name, correlationID, binaryDigest, bundle, preflight, changeSet, approval, authorization, importReceipt, transferReceipts, scanReceipts, gateResult, result)
	if err != nil {
...
func loadAndValidateExecutionInputs(options deploymentApplyOptions, at time.Time) (resources.DeploymentBundle, resources.TargetPreflightResult, resources.KubernetesChangeSet, resources.DeploymentApproval, resources.ExecutionAuthorization, resources.ArtifactImportReceipt, []resources.ArtifactTransferReceipt, []resources.ArtifactScanReceipt, *resources.AirgapProvenanceGateResult, string, error) {
	...
	if err != nil || !bundle.Validate().Valid {
		return bundle, resources.TargetPreflightResult{}, resources.KubernetesChangeSet{}, resources.DeploymentApproval{}, resources.ExecutionAuthorization{}, resources.ArtifactImportReceipt{}, nil, nil, nil, "YARA-EXE-101", errors.New("deployment bundle is invalid")
	}
	preflight, err := resources.LoadTargetPreflightResult(options.preflightPath)
	if err != nil || !preflight.Validate().Valid {
		return bundle, preflight, resources.KubernetesChangeSet{}, resources.DeploymentApproval{}, resources.ExecutionAuthorization{}, resources.ArtifactImportReceipt{}, nil, nil, nil, "YARA-EXE-102", errors.New("target preflight is invalid")
	}
	changeSet, err := resources.LoadKubernetesChangeSet(options.changeSetPath)
	if err != nil || !changeSet.Validate().Valid {
		return bundle, preflight, changeSet, resources.DeploymentApproval{}, resources.ExecutionAuthorization{}, resources.ArtifactImportReceipt{}, nil, nil, nil, "YARA-EXE-103", errors.New("change set is invalid")
	}
	approval, err := resources.LoadDeploymentApproval(options.approvalPath)
	if err != nil || !approval.Validate().Valid {
		return bundle, preflight, changeSet, approval, resources.ExecutionAuthorization{}, resources.ArtifactImportReceipt{}, nil, nil, nil, "YARA-EXE-104", errors.New("approval is invalid")
	}
	importReceipt, err := resources.LoadArtifactImportReceipt(options.importReceiptPath)
	if err != nil || !importReceipt.Validate().Valid {
		return bundle, preflight, changeSet, approval, resources.ExecutionAuthorization{}, importReceipt, nil, nil, nil, "YARA-EXE-116", errors.New("artifact import receipt is invalid")
	}
	authorization, err := resources.LoadExecutionAuthorization(options.authorizationPath)
	if err != nil || !authorization.Validate().Valid {
		return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-105", errors.New("execution authorization is invalid")
	}
	if approval.Spec.Decision != "approved" || approval.Spec.Effect != "review-only" || changeSet.Spec.Outcome != "review-required" {
		return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-106", errors.New("approved conflict-free review inputs are required")
	}
	approvalExpiry, _ := time.Parse(time.RFC3339Nano, approval.Spec.ExpiresAt)
	if !at.UTC().Before(approvalExpiry) {
		return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-107", errors.New("approval has expired")
	}
	...
	if issuedAt.Before(preflightAt) || issuedAt.Sub(preflightAt) > 15*time.Minute || issuedAt.Before(changeSetAt) || issuedAt.Sub(changeSetAt) > 5*time.Minute || issuedAt.Before(approvalAt) || authorizationExpiry.After(approvalExpiry) {
		return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-107", errors.New("authorization was not issued from fresh inputs within approval validity")
	}
	if bundle.Spec.PlanID != preflight.Spec.PlanID || bundle.Spec.PlanID != changeSet.Spec.PlanID || bundle.Spec.PlanID != approval.Spec.PlanID || bundle.Spec.PlanID != authorization.Spec.PlanID ||
		...
		return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-106", errors.New("execution inputs do not bind the same deployment")
	}
	if importReceipt.Spec.PlanID != bundle.Spec.PlanID || importReceipt.Spec.BundleID != bundle.Metadata.BundleID || importReceipt.Spec.Target != preflight.Spec.Target {
		return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-116", errors.New("import receipt does not bind the same plan, bundle and target")
	}
	if err := validateImportReceiptCoverage(bundle, importReceipt); err != nil {
		return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-116", err
	}
	...
	for _, path := range uniqueSortedStrings(options.transferReceiptPaths) {
		...
		if err != nil || !receipt.Validate().Valid {
			return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-117", errors.New("artifact transfer receipt is invalid")
		}
	...
	for _, path := range uniqueSortedStrings(options.scanReceiptPaths) {
		...
		if err != nil || !receipt.Validate().Valid {
			return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-118", errors.New("artifact scan receipt is invalid")
		}
	...
	if err != nil {
		return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-117", err
	}
	if requiresTransferChain {
		if gateResult != nil {
			...
			if err := validateScanReceiptChain(bundle, preflight, transferReceipts, scanReceipts); err != nil {
				return bundle, preflight, changeSet, approval, authorization, importReceipt, nil, nil, nil, "YARA-EXE-118", err
			}
		}
	}
	return bundle, preflight, changeSet, approval, authorization, importReceipt, transferReceipts, scanReceipts, gateResult, "", nil
}
