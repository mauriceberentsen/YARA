...
func lifecycleContract(args []string, stdout, stderr io.Writer) int {
	options, ok := parseContractOptions("contract lifecycle", args, stderr)
	if !ok {
		return ExitInvalidInput
	}
	snapshot, err := catalog.Load(options.catalogPath)
	if err != nil {
		return writeContractLifecycleFailure(stdout, options, []audit.Subject{attemptedInputSubject("CatalogSnapshot", options.catalogPath)}, "YARA-CAT-004", err, ExitInvalidInput)
	}
	catalogDigest, err := snapshot.Digest()
	if err != nil {
		return writeContractLifecycleFailure(stdout, options, nil, "YARA-CAT-500", err, ExitInternal)
	}
	catalogSubject := audit.Subject{Kind: "CatalogSnapshot", Digest: catalogDigest}
	target, found := snapshot.ContractTarget(options.assertionID)
	if !found {
		return writeContractLifecycleFailure(stdout, options, []audit.Subject{catalogSubject}, "YARA-CTR-107", fmt.Errorf("assertion %q is not a testable positive compatibility assertion", options.assertionID), ExitInvalidInput)
	}
	ctx := context.Background()
	artifactChecks, err := runtimeSmokeArtifactVerifier.Verify(ctx, target)
	if err != nil {
		return writeContractLifecycleFailure(stdout, options, []audit.Subject{catalogSubject}, "YARA-CTR-130", err, ExitInfeasible)
	}
	environment, err := contractProbe.Observe(ctx, options.target)
	if err != nil {
		return writeContractLifecycleFailure(stdout, options, []audit.Subject{catalogSubject}, "YARA-CTR-108", err, ExitInfeasible)
	}
	auditTarget := "ssh:" + environment.ReferenceDigest
	preflight, err := contracttest.Evaluate(options.name, catalogDigest, target, environment)
	if err != nil {
		return writeContractLifecycleFailureForTarget(stdout, options, auditTarget, []audit.Subject{catalogSubject}, "YARA-CTR-500", err, ExitInternal)
	}
	var lifecycleChecks []resources.ContractTestCheck
	if preflight.Spec.Outcome == "passed" && checksPassed(artifactChecks) {
		lifecycleChecks, err = lifecycleContractRunner.Run(ctx, options.target, target)
		if err != nil {
			return writeContractLifecycleFailureForTarget(stdout, options, auditTarget, []audit.Subject{catalogSubject}, "YARA-CTR-178", err, ExitInfeasible)
		}
	}
	...
	if report := result.Validate(); !report.Valid {
		return writeContractLifecycleReportFailure(stdout, options, auditTarget, []audit.Subject{catalogSubject}, report, ExitInternal)
	}
	data, err := yaml.Marshal(result)
	if err != nil {
		return writeContractLifecycleFailureForTarget(stdout, options, auditTarget, []audit.Subject{catalogSubject}, "YARA-CTR-500", fmt.Errorf("encode contract test result: %w", err), ExitInternal)
	}
	if err := writeExclusive(options.outputPath, data); err != nil {
		return writeContractLifecycleFailureForTarget(stdout, options, auditTarget, []audit.Subject{catalogSubject}, "YARA-CTR-005", err, ExitInvalidInput)
	}
	resultSubject := audit.Subject{Kind: "ContractTestResult", Digest: result.Metadata.ResultID}
	suffix, auditOutcome, exitCode := "completed", "success", ExitSuccess
	if result.Spec.Outcome == "blocked" {
	...
	if err := persistOperationAuditForTarget(options.auditPath, "contract.lifecycle", suffix, auditOutcome, auditTarget, []audit.Subject{catalogSubject, resultSubject}, contractDiagnosticCodes(result)); err != nil {
		_ = os.Remove(options.outputPath)
		return writeLoadError(stdout, "YARA-AUD-005", err)
	}
	encoder := json.NewEncoder(stdout)
