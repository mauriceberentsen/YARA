...
func newServeAPIHandler(snapshot catalog.Snapshot, catalogDigest string, report catalogcoverage.Report, uiEnabled bool, workspacePath string) (http.Handler, error) {
	...
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-index/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication index export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationIndexExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-055", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-055", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationIndexManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-055", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication index: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationIndex", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-index.export", "completed", manifest.Index.IndexState, "kubernetes:"+manifest.Index.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationIndexExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.IndexState = manifest.Index.IndexState
		response.Export.BlockerCode = manifest.Index.BlockerCode
	...
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-envelope/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication envelope export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationEnvelopeExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-056", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-056", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationEnvelopeManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-056", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication envelope: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationEnvelope", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-envelope.export", "completed", manifest.Envelope.EnvelopeState, "kubernetes:"+manifest.Envelope.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationEnvelopeExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.EnvelopeState = manifest.Envelope.EnvelopeState
		response.Export.BlockerCode = manifest.Envelope.BlockerCode
		writeServeJSON(writer, http.StatusOK, response)
	})
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-handoff/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication handoff export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationHandoffExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-057", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-057", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationHandoffManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-057", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication handoff: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationHandoff", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-handoff.export", "completed", manifest.Handoff.HandoffState, "kubernetes:"+manifest.Handoff.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationHandoffExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.HandoffState = manifest.Handoff.HandoffState
		response.Export.BlockerCode = manifest.Handoff.BlockerCode
		writeServeJSON(writer, http.StatusOK, response)
	})
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-acknowledgment/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication acknowledgment export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationAcknowledgmentExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-058", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-058", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationAcknowledgmentManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-058", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication acknowledgment: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationAcknowledgment", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-acknowledgment.export", "completed", manifest.Acknowledgment.AcknowledgmentState, "kubernetes:"+manifest.Acknowledgment.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationAcknowledgmentExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.AcknowledgmentState = manifest.Acknowledgment.AcknowledgmentState
		response.Export.BlockerCode = manifest.Acknowledgment.BlockerCode
	...
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-archive-index/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication archive index export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationArchiveIndexExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-059", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-059", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationArchiveIndexManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-059", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication archive index: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveIndex", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-archive-index.export", "completed", manifest.ArchiveIndex.ArchiveIndexState, "kubernetes:"+manifest.ArchiveIndex.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationArchiveIndexExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.ArchiveIndexState = manifest.ArchiveIndex.ArchiveIndexState
		response.Export.BlockerCode = manifest.ArchiveIndex.BlockerCode
		writeServeJSON(writer, http.StatusOK, response)
	})
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-archive-package/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication archive package export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationArchivePackageExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-060", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-060", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationArchivePackageManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-060", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication archive package: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationArchivePackage", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-archive-package.export", "completed", manifest.ArchivePackage.ArchivePackageState, "kubernetes:"+manifest.ArchivePackage.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationArchivePackageExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.ArchivePackageState = manifest.ArchivePackage.ArchivePackageState
		response.Export.BlockerCode = manifest.ArchivePackage.BlockerCode
		writeServeJSON(writer, http.StatusOK, response)
	})
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-archive-envelope/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication archive envelope export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationArchiveEnvelopeExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-061", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-061", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-061", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication archive envelope: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveEnvelope", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-archive-envelope.export", "completed", manifest.ArchiveEnvelope.ArchiveEnvelopeState, "kubernetes:"+manifest.ArchiveEnvelope.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationArchiveEnvelopeExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.ArchiveEnvelopeState = manifest.ArchiveEnvelope.ArchiveEnvelopeState
		response.Export.BlockerCode = manifest.ArchiveEnvelope.BlockerCode
		writeServeJSON(writer, http.StatusOK, response)
	})
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-archive-handoff/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication archive handoff export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationArchiveHandoffExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-062", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-062", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationArchiveHandoffManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-062", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication archive handoff: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveHandoff", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-archive-handoff.export", "completed", manifest.ArchiveHandoff.ArchiveHandoffState, "kubernetes:"+manifest.ArchiveHandoff.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationArchiveHandoffExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.ArchiveHandoffState = manifest.ArchiveHandoff.ArchiveHandoffState
		response.Export.BlockerCode = manifest.ArchiveHandoff.BlockerCode
		writeServeJSON(writer, http.StatusOK, response)
	})
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-archive-acknowledgment/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication archive acknowledgment export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationArchiveAcknowledgmentExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-063", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-063", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-063", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication archive acknowledgment: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveAcknowledgment", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-archive-acknowledgment.export", "completed", manifest.ArchiveAcknowledgment.ArchiveAcknowledgmentState, "kubernetes:"+manifest.ArchiveAcknowledgment.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.ArchiveAcknowledgmentState = manifest.ArchiveAcknowledgment.ArchiveAcknowledgmentState
		response.Export.BlockerCode = manifest.ArchiveAcknowledgment.BlockerCode
		writeServeJSON(writer, http.StatusOK, response)
	})
	apiMux.HandleFunc("/api/v1/workflow/rollout-closure/verify/publication-archive-attestation/export", func(writer http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodPost {
			writeServeNotFound(writer)
			return
		}
		if workspacePath == "" {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-009", "workflow rollout closure verify publication archive attestation export requires --workspace")
			return
		}
		payload, err := decodeWorkflowRolloutClosureVerifyPublicationArchiveAttestationExportRequest(request)
		if err != nil {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-064", err.Error())
			return
		}
		manifestPath, err := ensureWorkspaceFilePath(workspacePath, payload.ManifestPath, "manifestPath")
		if err != nil {
		...
		if manifestPath == auditPath {
			writeServeError(writer, http.StatusBadRequest, "YARA-SRV-064", "manifestPath and auditPath must be different files")
			return
		}
		manifest, subjects, err := buildWorkflowRolloutClosureVerifyPublicationArchiveAttestationManifest(workspacePath, payload)
		if err != nil {
			status := http.StatusBadRequest
			if gateErr, ok := err.(workflowGateError); ok {
				status = gateErr.Status
			}
			writeServeError(writer, status, "YARA-SRV-064", err.Error())
			return
		}
		manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
		if err != nil {
			writeServeError(writer, http.StatusInternalServerError, "YARA-SRV-500", fmt.Sprintf("encode rollout closure verify publication archive attestation: %v", err))
			return
		}
		manifestBytes = append(manifestBytes, '\n')
		if err := writeExclusive(manifestPath, manifestBytes); err != nil {
		...
		exportSubjects := append(append([]audit.Subject(nil), subjects...),
			audit.Subject{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveAttestation", Digest: digestBytes(manifestBytes)},
		)
		if err := persistOperationAuditForTarget(auditPath, "workflow.rollout-closure.verify.publication-archive-attestation.export", "completed", manifest.ArchiveAttestation.ArchiveAttestationState, "kubernetes:"+manifest.ArchiveAttestation.Continuity.TargetDigest, exportSubjects, nil); err != nil {
			_ = os.Remove(manifestPath)
			writeServeError(writer, http.StatusInternalServerError, "YARA-AUD-005", err.Error())
			return
		}
		response := workflowRolloutClosureVerifyPublicationArchiveAttestationExportResponse{Valid: true}
		response.Export.ManifestPath = manifestPath
		response.Export.AuditPath = auditPath
		response.Export.ArchiveAttestationState = manifest.ArchiveAttestation.ArchiveAttestationState
		response.Export.BlockerCode = manifest.ArchiveAttestation.BlockerCode
		writeServeJSON(writer, http.StatusOK, response)
	})
	var (
		uiFileSystem fs.FS
...
type workflowRolloutClosureVerifyPublicationIndexExportRequest struct {
	VerificationPublicationIndexReference string `json:"verificationPublicationIndexReference"`
	IndexedByReference                    string `json:"indexedByReference"`
	IndexedTimestamp                      string `json:"indexedTimestamp"`
	ManifestPath                          string `json:"manifestPath"`
	AuditPath                             string `json:"auditPath"`
}

type workflowRolloutClosureVerifyPublicationIndexExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath string `json:"manifestPath"`
		AuditPath    string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationIndexManifest struct {
	Valid bool `json:"valid"`
	Index struct {
		WorkspacePath                         string                    `json:"workspacePath"`
		VerificationPublicationIndexReference string                    `json:"verificationPublicationIndexReference"`
		...
		BlockerCode                           string                    `json:"blockerCode,omitempty"`
		Continuity                            workflowClosureContinuity `json:"continuity"`
		VerificationPublicationAttestation    workflowClosureArtifact   `json:"verificationPublicationAttestation"`
		VerificationPublicationPackage        workflowClosureArtifact   `json:"verificationPublicationPackage"`
		VerificationAttestationIndex          workflowClosureArtifact   `json:"verificationAttestationIndex"`
		VerificationAttestation               workflowClosureArtifact   `json:"verificationAttestation"`
		VerificationExportJSON                workflowClosureArtifact   `json:"verificationExportJson"`
		...
		PublicationChannel                    string                    `json:"publicationChannel"`
		PublicationLocationReference          string                    `json:"publicationLocationReference"`
	} `json:"rolloutClosureVerifyPublicationIndex"`
}

type workflowRolloutClosureVerifyPublicationEnvelopeExportRequest struct {
	VerificationPublicationEnvelopeReference string `json:"verificationPublicationEnvelopeReference"`
	DeliveredByReference                     string `json:"deliveredByReference"`
	DeliveryTimestamp                        string `json:"deliveryTimestamp"`
	DeliveryDestinationReference             string `json:"deliveryDestinationReference"`
	ManifestPath                             string `json:"manifestPath"`
	AuditPath                                string `json:"auditPath"`
}

type workflowRolloutClosureVerifyPublicationEnvelopeExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath  string `json:"manifestPath"`
		AuditPath     string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationEnvelopeManifest struct {
	Valid    bool `json:"valid"`
	Envelope struct {
		WorkspacePath                            string                    `json:"workspacePath"`
		VerificationPublicationEnvelopeReference string                    `json:"verificationPublicationEnvelopeReference"`
		...
		BlockerCode                              string                    `json:"blockerCode,omitempty"`
		Continuity                               workflowClosureContinuity `json:"continuity"`
		VerificationPublicationIndex             workflowClosureArtifact   `json:"verificationPublicationIndex"`
		VerificationPublicationAttestation       workflowClosureArtifact   `json:"verificationPublicationAttestation"`
		VerificationPublicationPackage           workflowClosureArtifact   `json:"verificationPublicationPackage"`
		VerificationAttestationIndex             workflowClosureArtifact   `json:"verificationAttestationIndex"`
		VerificationAttestation                  workflowClosureArtifact   `json:"verificationAttestation"`
		...
		PublicationChannel                       string                    `json:"publicationChannel"`
		PublicationLocationReference             string                    `json:"publicationLocationReference"`
	} `json:"rolloutClosureVerifyPublicationEnvelope"`
}

type workflowRolloutClosureVerifyPublicationHandoffExportRequest struct {
	VerificationPublicationHandoffReference string `json:"verificationPublicationHandoffReference"`
	ReceivedByReference                     string `json:"receivedByReference"`
	HandoffTimestamp                        string `json:"handoffTimestamp"`
	ManifestPath                            string `json:"manifestPath"`
	AuditPath                               string `json:"auditPath"`
}

type workflowRolloutClosureVerifyPublicationHandoffExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath string `json:"manifestPath"`
		AuditPath    string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationHandoffManifest struct {
	Valid   bool `json:"valid"`
	Handoff struct {
		WorkspacePath                            string                    `json:"workspacePath"`
		VerificationPublicationHandoffReference  string                    `json:"verificationPublicationHandoffReference"`
		...
		BlockerCode                              string                    `json:"blockerCode,omitempty"`
		Continuity                               workflowClosureContinuity `json:"continuity"`
		VerificationPublicationEnvelope          workflowClosureArtifact   `json:"verificationPublicationEnvelope"`
		VerificationPublicationIndex             workflowClosureArtifact   `json:"verificationPublicationIndex"`
		VerificationPublicationAttestation       workflowClosureArtifact   `json:"verificationPublicationAttestation"`
		VerificationPublicationPackage           workflowClosureArtifact   `json:"verificationPublicationPackage"`
		VerificationAttestationIndex             workflowClosureArtifact   `json:"verificationAttestationIndex"`
		...
		PublicationLocationReference             string                    `json:"publicationLocationReference"`
		DeliveryDestinationReference             string                    `json:"deliveryDestinationReference"`
	} `json:"rolloutClosureVerifyPublicationHandoff"`
}

type workflowRolloutClosureVerifyPublicationAcknowledgmentExportRequest struct {
	VerificationPublicationAcknowledgmentReference string `json:"verificationPublicationAcknowledgmentReference"`
	AcknowledgedByReference                        string `json:"acknowledgedByReference"`
	AcknowledgmentTimestamp                        string `json:"acknowledgmentTimestamp"`
	ManifestPath                                   string `json:"manifestPath"`
	AuditPath                                      string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationAcknowledgmentExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath        string `json:"manifestPath"`
		AuditPath           string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationAcknowledgmentManifest struct {
	Valid          bool `json:"valid"`
	Acknowledgment struct {
		WorkspacePath                                  string                    `json:"workspacePath"`
		VerificationPublicationAcknowledgmentReference string                    `json:"verificationPublicationAcknowledgmentReference"`
		...
		BlockerCode                                    string                    `json:"blockerCode,omitempty"`
		Continuity                                     workflowClosureContinuity `json:"continuity"`
		VerificationPublicationHandoff                 workflowClosureArtifact   `json:"verificationPublicationHandoff"`
		VerificationPublicationEnvelope                workflowClosureArtifact   `json:"verificationPublicationEnvelope"`
		VerificationPublicationIndex                   workflowClosureArtifact   `json:"verificationPublicationIndex"`
		VerificationPublicationAttestation             workflowClosureArtifact   `json:"verificationPublicationAttestation"`
		VerificationPublicationPackage                 workflowClosureArtifact   `json:"verificationPublicationPackage"`
		...
		PublicationLocationReference                   string                    `json:"publicationLocationReference"`
		DeliveryDestinationReference                   string                    `json:"deliveryDestinationReference"`
	} `json:"rolloutClosureVerifyPublicationAcknowledgment"`
}

type workflowRolloutClosureVerifyPublicationArchiveIndexExportRequest struct {
	VerificationPublicationArchiveIndexReference string `json:"verificationPublicationArchiveIndexReference"`
	IndexedByReference                           string `json:"indexedByReference"`
	IndexedTimestamp                             string `json:"indexedTimestamp"`
	ManifestPath                                 string `json:"manifestPath"`
	AuditPath                                    string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationArchiveIndexExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath      string `json:"manifestPath"`
		AuditPath         string `json:"auditPath"`
		ArchiveIndexState string `json:"archiveIndexState"`
		BlockerCode       string `json:"blockerCode,omitempty"`
	} `json:"export"`
}

type workflowRolloutClosureVerifyPublicationArchiveIndexManifest struct {
	Valid        bool `json:"valid"`
	ArchiveIndex struct {
		WorkspacePath                                  string                    `json:"workspacePath"`
		VerificationPublicationArchiveIndexReference   string                    `json:"verificationPublicationArchiveIndexReference"`
		...
		BlockerCode                                    string                    `json:"blockerCode,omitempty"`
		Continuity                                     workflowClosureContinuity `json:"continuity"`
		VerificationPublicationAcknowledgment          workflowClosureArtifact   `json:"verificationPublicationAcknowledgment"`
		VerificationPublicationHandoff                 workflowClosureArtifact   `json:"verificationPublicationHandoff"`
		VerificationPublicationEnvelope                workflowClosureArtifact   `json:"verificationPublicationEnvelope"`
		VerificationPublicationIndex                   workflowClosureArtifact   `json:"verificationPublicationIndex"`
		VerificationPublicationAttestation             workflowClosureArtifact   `json:"verificationPublicationAttestation"`
		...
		PublicationLocationReference                   string                    `json:"publicationLocationReference"`
		DeliveryDestinationReference                   string                    `json:"deliveryDestinationReference"`
	} `json:"rolloutClosureVerifyPublicationArchiveIndex"`
}

type workflowRolloutClosureVerifyPublicationArchivePackageExportRequest struct {
	VerificationPublicationArchivePackageReference string `json:"verificationPublicationArchivePackageReference"`
	PackagedByReference                            string `json:"packagedByReference"`
	PackagedTimestamp                              string `json:"packagedTimestamp"`
	ManifestPath                                   string `json:"manifestPath"`
	AuditPath                                      string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationArchivePackageExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath        string `json:"manifestPath"`
		AuditPath           string `json:"auditPath"`
		ArchivePackageState string `json:"archivePackageState"`
		BlockerCode         string `json:"blockerCode,omitempty"`
	} `json:"export"`
}

type workflowRolloutClosureVerifyPublicationArchivePackageManifest struct {
	Valid          bool `json:"valid"`
	ArchivePackage struct {
		WorkspacePath                                  string                    `json:"workspacePath"`
		VerificationPublicationArchivePackageReference string                    `json:"verificationPublicationArchivePackageReference"`
		PackagedByReference                            string                    `json:"packagedByReference"`
		PackagedTimestamp                              string                    `json:"packagedTimestamp"`
		...
		BlockerCode                                    string                    `json:"blockerCode,omitempty"`
		Continuity                                     workflowClosureContinuity `json:"continuity"`
		VerificationPublicationArchiveIndex            workflowClosureArtifact   `json:"verificationPublicationArchiveIndex"`
		VerificationPublicationAcknowledgment          workflowClosureArtifact   `json:"verificationPublicationAcknowledgment"`
		VerificationPublicationHandoff                 workflowClosureArtifact   `json:"verificationPublicationHandoff"`
		VerificationPublicationEnvelope                workflowClosureArtifact   `json:"verificationPublicationEnvelope"`
		VerificationPublicationIndex                   workflowClosureArtifact   `json:"verificationPublicationIndex"`
		...
		PublicationLocationReference                   string                    `json:"publicationLocationReference"`
		DeliveryDestinationReference                   string                    `json:"deliveryDestinationReference"`
	} `json:"rolloutClosureVerifyPublicationArchivePackage"`
}

type workflowRolloutClosureVerifyPublicationArchiveEnvelopeExportRequest struct {
	VerificationPublicationArchiveEnvelopeReference string `json:"verificationPublicationArchiveEnvelopeReference"`
	DeliveredByReference                            string `json:"deliveredByReference"`
	DeliveryTimestamp                               string `json:"deliveryTimestamp"`
	DeliveryDestinationReference                    string `json:"deliveryDestinationReference"`
	ManifestPath                                    string `json:"manifestPath"`
...
type workflowRolloutClosureVerifyPublicationArchiveEnvelopeExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath         string `json:"manifestPath"`
		AuditPath            string `json:"auditPath"`
		ArchiveEnvelopeState string `json:"archiveEnvelopeState"`
		BlockerCode          string `json:"blockerCode,omitempty"`
	} `json:"export"`
}

type workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest struct {
	Valid           bool `json:"valid"`
	ArchiveEnvelope struct {
		WorkspacePath                                   string                    `json:"workspacePath"`
		VerificationPublicationArchiveEnvelopeReference string                    `json:"verificationPublicationArchiveEnvelopeReference"`
		DeliveredByReference                            string                    `json:"deliveredByReference"`
		DeliveryTimestamp                               string                    `json:"deliveryTimestamp"`
		...
		BlockerCode                                     string                    `json:"blockerCode,omitempty"`
		Continuity                                      workflowClosureContinuity `json:"continuity"`
		VerificationPublicationArchivePackage           workflowClosureArtifact   `json:"verificationPublicationArchivePackage"`
		VerificationPublicationArchiveIndex             workflowClosureArtifact   `json:"verificationPublicationArchiveIndex"`
		VerificationPublicationAcknowledgment           workflowClosureArtifact   `json:"verificationPublicationAcknowledgment"`
		VerificationPublicationHandoff                  workflowClosureArtifact   `json:"verificationPublicationHandoff"`
		VerificationPublicationEnvelope                 workflowClosureArtifact   `json:"verificationPublicationEnvelope"`
		...
		PublicationChannel                              string                    `json:"publicationChannel"`
		PublicationLocationReference                    string                    `json:"publicationLocationReference"`
	} `json:"rolloutClosureVerifyPublicationArchiveEnvelope"`
}

type workflowRolloutClosureVerifyPublicationArchiveHandoffExportRequest struct {
	VerificationPublicationArchiveHandoffReference string `json:"verificationPublicationArchiveHandoffReference"`
	ReceivedByReference                            string `json:"receivedByReference"`
	HandoffTimestamp                               string `json:"handoffTimestamp"`
	ManifestPath                                   string `json:"manifestPath"`
	AuditPath                                      string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationArchiveHandoffExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath        string `json:"manifestPath"`
		AuditPath           string `json:"auditPath"`
		ArchiveHandoffState string `json:"archiveHandoffState"`
		BlockerCode         string `json:"blockerCode,omitempty"`
	} `json:"export"`
}

type workflowRolloutClosureVerifyPublicationArchiveHandoffManifest struct {
	Valid          bool `json:"valid"`
	ArchiveHandoff struct {
		WorkspacePath                                   string                    `json:"workspacePath"`
		VerificationPublicationArchiveHandoffReference  string                    `json:"verificationPublicationArchiveHandoffReference"`
		ReceivedByReference                             string                    `json:"receivedByReference"`
		HandoffTimestamp                                string                    `json:"handoffTimestamp"`
		...
		BlockerCode                                     string                    `json:"blockerCode,omitempty"`
		Continuity                                      workflowClosureContinuity `json:"continuity"`
		VerificationPublicationArchiveEnvelope          workflowClosureArtifact   `json:"verificationPublicationArchiveEnvelope"`
		VerificationPublicationArchivePackage           workflowClosureArtifact   `json:"verificationPublicationArchivePackage"`
		VerificationPublicationArchiveIndex             workflowClosureArtifact   `json:"verificationPublicationArchiveIndex"`
		VerificationPublicationAcknowledgment           workflowClosureArtifact   `json:"verificationPublicationAcknowledgment"`
		VerificationPublicationHandoff                  workflowClosureArtifact   `json:"verificationPublicationHandoff"`
		...
		PublicationLocationReference                    string                    `json:"publicationLocationReference"`
		DeliveryDestinationReference                    string                    `json:"deliveryDestinationReference"`
	} `json:"rolloutClosureVerifyPublicationArchiveHandoff"`
}

type workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentExportRequest struct {
	VerificationPublicationArchiveAcknowledgmentReference string `json:"verificationPublicationArchiveAcknowledgmentReference"`
	AcknowledgedByReference                               string `json:"acknowledgedByReference"`
	AcknowledgmentTimestamp                               string `json:"acknowledgmentTimestamp"`
	ManifestPath                                          string `json:"manifestPath"`
	AuditPath                                             string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath               string `json:"manifestPath"`
		AuditPath                  string `json:"auditPath"`
		ArchiveAcknowledgmentState string `json:"archiveAcknowledgmentState"`
		BlockerCode                string `json:"blockerCode,omitempty"`
	} `json:"export"`
}

type workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest struct {
	Valid                 bool `json:"valid"`
	ArchiveAcknowledgment struct {
		WorkspacePath                                         string                    `json:"workspacePath"`
		VerificationPublicationArchiveAcknowledgmentReference string                    `json:"verificationPublicationArchiveAcknowledgmentReference"`
		AcknowledgedByReference                               string                    `json:"acknowledgedByReference"`
		AcknowledgmentTimestamp                               string                    `json:"acknowledgmentTimestamp"`
		...
		BlockerCode                                           string                    `json:"blockerCode,omitempty"`
		Continuity                                            workflowClosureContinuity `json:"continuity"`
		VerificationPublicationArchiveHandoff                 workflowClosureArtifact   `json:"verificationPublicationArchiveHandoff"`
		VerificationPublicationArchiveEnvelope                workflowClosureArtifact   `json:"verificationPublicationArchiveEnvelope"`
		VerificationPublicationArchivePackage                 workflowClosureArtifact   `json:"verificationPublicationArchivePackage"`
		VerificationPublicationArchiveIndex                   workflowClosureArtifact   `json:"verificationPublicationArchiveIndex"`
		VerificationPublicationAcknowledgment                 workflowClosureArtifact   `json:"verificationPublicationAcknowledgment"`
		...
		PublicationLocationReference                          string                    `json:"publicationLocationReference"`
		DeliveryDestinationReference                          string                    `json:"deliveryDestinationReference"`
	} `json:"rolloutClosureVerifyPublicationArchiveAcknowledgment"`
}

type workflowRolloutClosureVerifyPublicationArchiveAttestationExportRequest struct {
	VerificationPublicationArchiveAttestationReference string `json:"verificationPublicationArchiveAttestationReference"`
	AttestedByReference                                string `json:"attestedByReference"`
	AttestationTimestamp                               string `json:"attestationTimestamp"`
	ManifestPath                                       string `json:"manifestPath"`
	AuditPath                                          string `json:"auditPath"`
...
type workflowRolloutClosureVerifyPublicationArchiveAttestationExportResponse struct {
	Valid  bool `json:"valid"`
	Export struct {
		ManifestPath            string `json:"manifestPath"`
		AuditPath               string `json:"auditPath"`
		ArchiveAttestationState string `json:"archiveAttestationState"`
		BlockerCode             string `json:"blockerCode,omitempty"`
	} `json:"export"`
}

type workflowRolloutClosureVerifyPublicationArchiveAttestationManifest struct {
	Valid              bool `json:"valid"`
	ArchiveAttestation struct {
		WorkspacePath                                         string                    `json:"workspacePath"`
		VerificationPublicationArchiveAttestationReference    string                    `json:"verificationPublicationArchiveAttestationReference"`
		AttestedByReference                                   string                    `json:"attestedByReference"`
		AttestationTimestamp                                  string                    `json:"attestationTimestamp"`
		...
		BlockerCode                                           string                    `json:"blockerCode,omitempty"`
		Continuity                                            workflowClosureContinuity `json:"continuity"`
		VerificationPublicationArchiveAcknowledgment          workflowClosureArtifact   `json:"verificationPublicationArchiveAcknowledgment"`
		VerificationPublicationArchiveHandoff                 workflowClosureArtifact   `json:"verificationPublicationArchiveHandoff"`
		VerificationPublicationArchiveEnvelope                workflowClosureArtifact   `json:"verificationPublicationArchiveEnvelope"`
		VerificationPublicationArchivePackage                 workflowClosureArtifact   `json:"verificationPublicationArchivePackage"`
		VerificationPublicationArchiveIndex                   workflowClosureArtifact   `json:"verificationPublicationArchiveIndex"`
		...
		PublicationLocationReference                          string                    `json:"publicationLocationReference"`
		DeliveryDestinationReference                          string                    `json:"deliveryDestinationReference"`
	} `json:"rolloutClosureVerifyPublicationArchiveAttestation"`
}
...
func decodeWorkflowRolloutClosureVerifyPublicationIndexExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationIndexExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationIndexExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationIndexReference) == "" ||
		strings.TrimSpace(payload.IndexedByReference) == "" ||
		strings.TrimSpace(payload.IndexedTimestamp) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationIndexReference, indexedByReference, indexedTimestamp, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
	if _, err := time.Parse(time.RFC3339Nano, payload.IndexedTimestamp); err != nil {
		return payload, errors.New("indexedTimestamp must be a valid RFC3339 timestamp")
	}
	return payload, nil
}

func decodeWorkflowRolloutClosureVerifyPublicationEnvelopeExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationEnvelopeExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationEnvelopeExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationEnvelopeReference) == "" ||
		strings.TrimSpace(payload.DeliveredByReference) == "" ||
		strings.TrimSpace(payload.DeliveryTimestamp) == "" ||
		strings.TrimSpace(payload.DeliveryDestinationReference) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationEnvelopeReference, deliveredByReference, deliveryTimestamp, deliveryDestinationReference, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
...
func decodeWorkflowRolloutClosureVerifyPublicationHandoffExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationHandoffExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationHandoffExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationHandoffReference) == "" ||
		strings.TrimSpace(payload.ReceivedByReference) == "" ||
		strings.TrimSpace(payload.HandoffTimestamp) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationHandoffReference, receivedByReference, handoffTimestamp, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
...
func decodeWorkflowRolloutClosureVerifyPublicationAcknowledgmentExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationAcknowledgmentExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationAcknowledgmentExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationAcknowledgmentReference) == "" ||
		strings.TrimSpace(payload.AcknowledgedByReference) == "" ||
		strings.TrimSpace(payload.AcknowledgmentTimestamp) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationAcknowledgmentReference, acknowledgedByReference, acknowledgmentTimestamp, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
...
func decodeWorkflowRolloutClosureVerifyPublicationArchiveIndexExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationArchiveIndexExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationArchiveIndexExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationArchiveIndexReference) == "" ||
		strings.TrimSpace(payload.IndexedByReference) == "" ||
		strings.TrimSpace(payload.IndexedTimestamp) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationArchiveIndexReference, indexedByReference, indexedTimestamp, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
...
func decodeWorkflowRolloutClosureVerifyPublicationArchivePackageExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationArchivePackageExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationArchivePackageExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationArchivePackageReference) == "" ||
		strings.TrimSpace(payload.PackagedByReference) == "" ||
		strings.TrimSpace(payload.PackagedTimestamp) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationArchivePackageReference, packagedByReference, packagedTimestamp, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
...
func decodeWorkflowRolloutClosureVerifyPublicationArchiveEnvelopeExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationArchiveEnvelopeExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationArchiveEnvelopeExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationArchiveEnvelopeReference) == "" ||
		strings.TrimSpace(payload.DeliveredByReference) == "" ||
		strings.TrimSpace(payload.DeliveryTimestamp) == "" ||
		strings.TrimSpace(payload.DeliveryDestinationReference) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationArchiveEnvelopeReference, deliveredByReference, deliveryTimestamp, deliveryDestinationReference, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
...
func decodeWorkflowRolloutClosureVerifyPublicationArchiveHandoffExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationArchiveHandoffExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationArchiveHandoffExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationArchiveHandoffReference) == "" ||
		strings.TrimSpace(payload.ReceivedByReference) == "" ||
		strings.TrimSpace(payload.HandoffTimestamp) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationArchiveHandoffReference, receivedByReference, handoffTimestamp, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
...
func decodeWorkflowRolloutClosureVerifyPublicationArchiveAcknowledgmentExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationArchiveAcknowledgmentReference) == "" ||
		strings.TrimSpace(payload.AcknowledgedByReference) == "" ||
		strings.TrimSpace(payload.AcknowledgmentTimestamp) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationArchiveAcknowledgmentReference, acknowledgedByReference, acknowledgmentTimestamp, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
...
func decodeWorkflowRolloutClosureVerifyPublicationArchiveAttestationExportRequest(request *http.Request) (workflowRolloutClosureVerifyPublicationArchiveAttestationExportRequest, error) {
	var payload workflowRolloutClosureVerifyPublicationArchiveAttestationExportRequest
	decoder := json.NewDecoder(request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&payload); err != nil {
		return payload, fmt.Errorf("decode request body: %w", err)
	...
	if strings.TrimSpace(payload.VerificationPublicationArchiveAttestationReference) == "" ||
		strings.TrimSpace(payload.AttestedByReference) == "" ||
		strings.TrimSpace(payload.AttestationTimestamp) == "" ||
		strings.TrimSpace(payload.ManifestPath) == "" ||
		strings.TrimSpace(payload.AuditPath) == "" {
		return payload, errors.New("verificationPublicationArchiveAttestationReference, attestedByReference, attestationTimestamp, manifestPath and auditPath are required")
	}
	if payload.ManifestPath == payload.AuditPath {
		return payload, errors.New("manifestPath and auditPath must be different files")
	}
...
func loadLatestRolloutClosureVerifyPublicationAttestation(workspacePath string) (workflowRolloutClosureVerifyPublicationAttestationManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationAttestationExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationAttestationManifest{}, "", "", errors.New("YARA-RCVPI-001: verification publication index export requires at least one closure verification publication attestation manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationAttestationManifest{}, "", "", fmt.Errorf("read latest verification publication attestation %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationAttestationManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationAttestationManifest{}, "", "", fmt.Errorf("decode latest verification publication attestation %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationAttestationManifest{}, "", "", errors.New("YARA-RCVPI-002: latest closure verification publication attestation manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}

func loadLatestRolloutClosureVerifyPublicationIndex(workspacePath string) (workflowRolloutClosureVerifyPublicationIndexManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationIndexExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, "", "", errors.New("YARA-RCVPE-001: verification publication envelope export requires at least one closure verification publication index manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, "", "", fmt.Errorf("read latest verification publication index %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationIndexManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, "", "", fmt.Errorf("decode latest verification publication index %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, "", "", errors.New("YARA-RCVPE-002: latest closure verification publication index manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}

func loadLatestRolloutClosureVerifyPublicationEnvelope(workspacePath string) (workflowRolloutClosureVerifyPublicationEnvelopeManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationEnvelopeExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, "", "", errors.New("YARA-RCVPH-001: verification publication handoff export requires at least one closure verification publication envelope manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, "", "", fmt.Errorf("read latest verification publication envelope %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationEnvelopeManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, "", "", fmt.Errorf("decode latest verification publication envelope %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, "", "", errors.New("YARA-RCVPH-002: latest closure verification publication envelope manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}

func loadLatestRolloutClosureVerifyPublicationHandoff(workspacePath string) (workflowRolloutClosureVerifyPublicationHandoffManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationHandoffExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, "", "", errors.New("YARA-RCVPAK-001: verification publication acknowledgment export requires at least one closure verification publication handoff manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, "", "", fmt.Errorf("read latest verification publication handoff %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationHandoffManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, "", "", fmt.Errorf("decode latest verification publication handoff %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, "", "", errors.New("YARA-RCVPAK-002: latest closure verification publication handoff manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}

func loadLatestRolloutClosureVerifyPublicationAcknowledgment(workspacePath string) (workflowRolloutClosureVerifyPublicationAcknowledgmentManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationAcknowledgmentExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, "", "", errors.New("YARA-RCVPAX-001: verification publication archive index export requires at least one closure verification publication acknowledgment manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, "", "", fmt.Errorf("read latest verification publication acknowledgment %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, "", "", fmt.Errorf("decode latest verification publication acknowledgment %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, "", "", errors.New("YARA-RCVPAX-002: latest closure verification publication acknowledgment manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}

func loadLatestRolloutClosureVerifyPublicationArchiveIndex(workspacePath string) (workflowRolloutClosureVerifyPublicationArchiveIndexManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationArchiveIndexExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, "", "", errors.New("YARA-RCVPAP-001: verification publication archive package export requires at least one closure verification publication archive index manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, "", "", fmt.Errorf("read latest verification publication archive index %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, "", "", fmt.Errorf("decode latest verification publication archive index %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, "", "", errors.New("YARA-RCVPAP-002: latest closure verification publication archive index manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}

func loadLatestRolloutClosureVerifyPublicationArchivePackage(workspacePath string) (workflowRolloutClosureVerifyPublicationArchivePackageManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationArchivePackageExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, "", "", errors.New("YARA-RCVPAE-001: verification publication archive envelope export requires at least one closure verification publication archive package manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, "", "", fmt.Errorf("read latest verification publication archive package %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationArchivePackageManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, "", "", fmt.Errorf("decode latest verification publication archive package %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, "", "", errors.New("YARA-RCVPAE-002: latest closure verification publication archive package manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}

func loadLatestRolloutClosureVerifyPublicationArchiveEnvelope(workspacePath string) (workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationArchiveEnvelopeExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, "", "", errors.New("YARA-RCVPAH-001: verification publication archive handoff export requires at least one closure verification publication archive envelope manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, "", "", fmt.Errorf("read latest verification publication archive envelope %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, "", "", fmt.Errorf("decode latest verification publication archive envelope %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, "", "", errors.New("YARA-RCVPAH-002: latest closure verification publication archive envelope manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}

func loadLatestRolloutClosureVerifyPublicationArchiveHandoff(workspacePath string) (workflowRolloutClosureVerifyPublicationArchiveHandoffManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationArchiveHandoffExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, "", "", errors.New("YARA-RCVPAA-001: verification publication archive acknowledgment export requires at least one closure verification publication archive handoff manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, "", "", fmt.Errorf("read latest verification publication archive handoff %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, "", "", fmt.Errorf("decode latest verification publication archive handoff %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, "", "", errors.New("YARA-RCVPAA-002: latest closure verification publication archive handoff manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}

func loadLatestRolloutClosureVerifyPublicationArchiveAcknowledgment(workspacePath string) (workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest, string, string, error) {
	paths := discoverRolloutClosureVerifyPublicationArchiveAcknowledgmentExports(workspacePath)
	if len(paths) == 0 {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, "", "", errors.New("YARA-RCVPAT-001: verification publication archive attestation export requires at least one closure verification publication archive acknowledgment manifest")
	}
	latestPath := paths[len(paths)-1]
	content, err := os.ReadFile(latestPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, "", "", fmt.Errorf("read latest verification publication archive acknowledgment %s: %w", filepath.Base(latestPath), err)
	}
	manifest := workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}
	if err := json.Unmarshal(content, &manifest); err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, "", "", fmt.Errorf("decode latest verification publication archive acknowledgment %s: %w", filepath.Base(latestPath), err)
	}
	if !manifest.Valid {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, "", "", errors.New("YARA-RCVPAT-002: latest closure verification publication archive acknowledgment manifest is invalid")
	}
	return manifest, latestPath, digestBytes(content), nil
}
...
func buildWorkflowRolloutClosureVerifyPublicationIndexManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationIndexExportRequest) (workflowRolloutClosureVerifyPublicationIndexManifest, []audit.Subject, error) {
	publicationAttestation, publicationAttestationPath, publicationAttestationDigest, err := loadLatestRolloutClosureVerifyPublicationAttestation(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationPackage, publicationPackagePath, publicationPackageDigest, err := loadLatestRolloutClosureVerifyPublicationPackage(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPI-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPI-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationAttestation.Publication.PublicationState == "publication-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPI-005: latest verification publication attestation is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationAttestation.Publication.Continuity != publicationPackage.Package.Continuity ||
		publicationAttestation.Publication.Continuity != attestationIndex.Index.Continuity ||
		publicationAttestation.Publication.Continuity != attestation.Attestation.Continuity ||
		publicationAttestation.Publication.Continuity != verifyExport.Export.Continuity ||
		publicationAttestation.Publication.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPI-006: latest publication-attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationAttestation.Publication.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPI-007: latest verification publication attestation does not bind the latest publication package digest",
		}
	}
	if publicationAttestation.Publication.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPI-008: latest verification publication attestation does not bind the latest attestation index digest",
		}
	}
	if publicationAttestation.Publication.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPI-009: latest verification publication attestation does not bind the latest verification attestation digest",
		}
	}
	if publicationAttestation.Publication.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPI-010: latest verification publication attestation does not bind the latest verification export json digest",
		}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationAttestation.Publication.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPI-011: latest verification publication attestation does not bind the latest verification export markdown digest",
		}
	}
	indexState := "index-ready"
	blockerCode := ""
	if publicationAttestation.Publication.PublicationState == "publication-blocked" ||
		publicationPackage.Package.PackageState == "package-blocked" ||
		attestationIndex.Index.IndexState == "index-blocked" ||
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		indexState = "index-blocked"
		blockerCode = mapValueOrDefault(publicationAttestation.Publication.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPI-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationIndexManifest{Valid: true}
	manifest.Index.WorkspacePath = workspacePath
	manifest.Index.VerificationPublicationIndexReference = strings.TrimSpace(payload.VerificationPublicationIndexReference)
	manifest.Index.IndexedByReference = strings.TrimSpace(payload.IndexedByReference)
	manifest.Index.IndexedTimestamp = strings.TrimSpace(payload.IndexedTimestamp)
	manifest.Index.IndexState = indexState
	manifest.Index.BlockerCode = blockerCode
	manifest.Index.Continuity = publicationAttestation.Publication.Continuity
	manifest.Index.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.Index.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.Index.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.Index.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.Index.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.Index.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.Index.VerificationPublicationReference = publicationAttestation.Publication.VerificationPublicationReference
	manifest.Index.VerificationPackageReference = publicationAttestation.Publication.VerificationPackageReference
	manifest.Index.AttestationIndexReference = publicationAttestation.Publication.AttestationIndexReference
	manifest.Index.AttestationReference = publicationAttestation.Publication.AttestationReference
	manifest.Index.VerificationReference = publicationAttestation.Publication.VerificationReference
	manifest.Index.OperatorReference = publicationAttestation.Publication.OperatorReference
	manifest.Index.VerificationTimestamp = publicationAttestation.Publication.VerificationTimestamp
	manifest.Index.PublicationChannel = publicationAttestation.Publication.PublicationChannel
	manifest.Index.PublicationLocationReference = publicationAttestation.Publication.PublicationLocationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationAttestation", Digest: publicationAttestationDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationPackage", Digest: publicationPackageDigest},
		{Kind: "WorkflowRolloutClosureVerifyAttestationIndexManifest", Digest: attestationIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyAttestation", Digest: attestationDigest},
		{Kind: "WorkflowRolloutClosureVerifyJSON", Digest: verifyExportDigest},
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationIndexReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.Index.VerificationPublicationIndexReference,
				manifest.Index.IndexedByReference,
				manifest.Index.IndexedTimestamp,
				manifest.Index.VerificationPublicationReference,
				manifest.Index.VerificationPackageReference,
				manifest.Index.AttestationIndexReference,
				manifest.Index.AttestationReference,
				manifest.Index.VerificationReference,
				manifest.Index.Continuity.AuthorizationID,
...
func buildWorkflowRolloutClosureVerifyPublicationEnvelopeManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationEnvelopeExportRequest) (workflowRolloutClosureVerifyPublicationEnvelopeManifest, []audit.Subject, error) {
	publicationIndex, publicationIndexPath, publicationIndexDigest, err := loadLatestRolloutClosureVerifyPublicationIndex(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationAttestation, publicationAttestationPath, publicationAttestationDigest, err := loadLatestRolloutClosureVerifyPublicationAttestation(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationIndex.Index.IndexState == "index-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-005: latest verification publication index is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationIndex.Index.Continuity != publicationAttestation.Publication.Continuity ||
		publicationIndex.Index.Continuity != publicationPackage.Package.Continuity ||
		publicationIndex.Index.Continuity != attestationIndex.Index.Continuity ||
		publicationIndex.Index.Continuity != attestation.Attestation.Continuity ||
		publicationIndex.Index.Continuity != verifyExport.Export.Continuity ||
		publicationIndex.Index.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-006: latest publication-index/attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationIndex.Index.VerificationPublicationAttestation.Digest != publicationAttestationDigest {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-007: latest verification publication index does not bind the latest publication attestation digest",
		}
	}
	if publicationIndex.Index.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-008: latest verification publication index does not bind the latest publication package digest",
		}
	}
	if publicationIndex.Index.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-009: latest verification publication index does not bind the latest attestation index digest",
		}
	}
	if publicationIndex.Index.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-010: latest verification publication index does not bind the latest verification attestation digest",
		}
	}
	if publicationIndex.Index.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-011: latest verification publication index does not bind the latest verification export json digest",
		}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationIndex.Index.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPE-012: latest verification publication index does not bind the latest verification export markdown digest",
		}
	}
	envelopeState := "envelope-ready"
	blockerCode := ""
	if publicationIndex.Index.IndexState == "index-blocked" ||
		publicationAttestation.Publication.PublicationState == "publication-blocked" ||
		publicationPackage.Package.PackageState == "package-blocked" ||
		attestationIndex.Index.IndexState == "index-blocked" ||
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		envelopeState = "envelope-blocked"
		blockerCode = mapValueOrDefault(publicationIndex.Index.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPE-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationEnvelopeManifest{Valid: true}
	manifest.Envelope.WorkspacePath = workspacePath
	manifest.Envelope.VerificationPublicationEnvelopeReference = strings.TrimSpace(payload.VerificationPublicationEnvelopeReference)
	manifest.Envelope.DeliveredByReference = strings.TrimSpace(payload.DeliveredByReference)
	manifest.Envelope.DeliveryTimestamp = strings.TrimSpace(payload.DeliveryTimestamp)
	manifest.Envelope.DeliveryDestinationReference = strings.TrimSpace(payload.DeliveryDestinationReference)
	manifest.Envelope.EnvelopeState = envelopeState
	manifest.Envelope.BlockerCode = blockerCode
	manifest.Envelope.Continuity = publicationIndex.Index.Continuity
	manifest.Envelope.VerificationPublicationIndex = workflowClosureArtifact{Path: publicationIndexPath, Digest: publicationIndexDigest}
	manifest.Envelope.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.Envelope.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.Envelope.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.Envelope.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.Envelope.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.Envelope.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.Envelope.VerificationPublicationIndexReference = publicationIndex.Index.VerificationPublicationIndexReference
	manifest.Envelope.VerificationPublicationReference = publicationIndex.Index.VerificationPublicationReference
	manifest.Envelope.VerificationPackageReference = publicationIndex.Index.VerificationPackageReference
	manifest.Envelope.AttestationIndexReference = publicationIndex.Index.AttestationIndexReference
	manifest.Envelope.AttestationReference = publicationIndex.Index.AttestationReference
	manifest.Envelope.VerificationReference = publicationIndex.Index.VerificationReference
	manifest.Envelope.OperatorReference = publicationIndex.Index.OperatorReference
	manifest.Envelope.VerificationTimestamp = publicationIndex.Index.VerificationTimestamp
	manifest.Envelope.PublicationChannel = publicationIndex.Index.PublicationChannel
	manifest.Envelope.PublicationLocationReference = publicationIndex.Index.PublicationLocationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationIndex", Digest: publicationIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationAttestation", Digest: publicationAttestationDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationPackage", Digest: publicationPackageDigest},
		{Kind: "WorkflowRolloutClosureVerifyAttestationIndexManifest", Digest: attestationIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyAttestation", Digest: attestationDigest},
		...
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationEnvelopeReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.Envelope.VerificationPublicationEnvelopeReference,
				manifest.Envelope.DeliveredByReference,
				manifest.Envelope.DeliveryTimestamp,
				manifest.Envelope.DeliveryDestinationReference,
				manifest.Envelope.VerificationPublicationIndexReference,
				manifest.Envelope.VerificationPublicationReference,
				manifest.Envelope.VerificationPackageReference,
				manifest.Envelope.AttestationIndexReference,
				manifest.Envelope.AttestationReference,
				manifest.Envelope.VerificationReference,
				manifest.Envelope.Continuity.AuthorizationID,
				manifest.Envelope.Continuity.TargetDigest,
				manifest.Envelope.BlockerCode,
			}, "|"))),
		},
	}
	return manifest, subjects, nil
...
func buildWorkflowRolloutClosureVerifyPublicationHandoffManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationHandoffExportRequest) (workflowRolloutClosureVerifyPublicationHandoffManifest, []audit.Subject, error) {
	publicationEnvelope, publicationEnvelopePath, publicationEnvelopeDigest, err := loadLatestRolloutClosureVerifyPublicationEnvelope(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationIndex, publicationIndexPath, publicationIndexDigest, err := loadLatestRolloutClosureVerifyPublicationIndex(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationEnvelope.Envelope.EnvelopeState == "envelope-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-005: latest verification publication envelope is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationEnvelope.Envelope.Continuity != publicationIndex.Index.Continuity ||
		publicationEnvelope.Envelope.Continuity != publicationAttestation.Publication.Continuity ||
		publicationEnvelope.Envelope.Continuity != publicationPackage.Package.Continuity ||
		publicationEnvelope.Envelope.Continuity != attestationIndex.Index.Continuity ||
		publicationEnvelope.Envelope.Continuity != attestation.Attestation.Continuity ||
		publicationEnvelope.Envelope.Continuity != verifyExport.Export.Continuity ||
		publicationEnvelope.Envelope.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-006: latest publication-envelope/index/attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationEnvelope.Envelope.VerificationPublicationIndex.Digest != publicationIndexDigest {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-007: latest verification publication envelope does not bind the latest publication index digest",
		}
	}
	if publicationEnvelope.Envelope.VerificationPublicationAttestation.Digest != publicationAttestationDigest {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-008: latest verification publication envelope does not bind the latest publication attestation digest",
		}
	}
	if publicationEnvelope.Envelope.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-009: latest verification publication envelope does not bind the latest publication package digest",
		}
	}
	if publicationEnvelope.Envelope.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-010: latest verification publication envelope does not bind the latest attestation index digest",
		}
	}
	if publicationEnvelope.Envelope.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-011: latest verification publication envelope does not bind the latest verification attestation digest",
		}
	}
	if publicationEnvelope.Envelope.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-012: latest verification publication envelope does not bind the latest verification export json digest",
		}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationEnvelope.Envelope.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPH-013: latest verification publication envelope does not bind the latest verification export markdown digest",
		}
	}
	handoffState := "handoff-ready"
	blockerCode := ""
	if publicationEnvelope.Envelope.EnvelopeState == "envelope-blocked" ||
		publicationIndex.Index.IndexState == "index-blocked" ||
		publicationAttestation.Publication.PublicationState == "publication-blocked" ||
		publicationPackage.Package.PackageState == "package-blocked" ||
		attestationIndex.Index.IndexState == "index-blocked" ||
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		handoffState = "handoff-blocked"
		blockerCode = mapValueOrDefault(publicationEnvelope.Envelope.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPH-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationHandoffManifest{Valid: true}
	manifest.Handoff.WorkspacePath = workspacePath
	manifest.Handoff.VerificationPublicationHandoffReference = strings.TrimSpace(payload.VerificationPublicationHandoffReference)
	manifest.Handoff.ReceivedByReference = strings.TrimSpace(payload.ReceivedByReference)
	manifest.Handoff.HandoffTimestamp = strings.TrimSpace(payload.HandoffTimestamp)
	manifest.Handoff.HandoffState = handoffState
	manifest.Handoff.BlockerCode = blockerCode
	manifest.Handoff.Continuity = publicationEnvelope.Envelope.Continuity
	manifest.Handoff.VerificationPublicationEnvelope = workflowClosureArtifact{Path: publicationEnvelopePath, Digest: publicationEnvelopeDigest}
	manifest.Handoff.VerificationPublicationIndex = workflowClosureArtifact{Path: publicationIndexPath, Digest: publicationIndexDigest}
	manifest.Handoff.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.Handoff.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.Handoff.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.Handoff.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.Handoff.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.Handoff.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.Handoff.VerificationPublicationEnvelopeReference = publicationEnvelope.Envelope.VerificationPublicationEnvelopeReference
	manifest.Handoff.VerificationPublicationIndexReference = publicationEnvelope.Envelope.VerificationPublicationIndexReference
	manifest.Handoff.VerificationPublicationReference = publicationEnvelope.Envelope.VerificationPublicationReference
	manifest.Handoff.VerificationPackageReference = publicationEnvelope.Envelope.VerificationPackageReference
	manifest.Handoff.AttestationIndexReference = publicationEnvelope.Envelope.AttestationIndexReference
	manifest.Handoff.AttestationReference = publicationEnvelope.Envelope.AttestationReference
	manifest.Handoff.VerificationReference = publicationEnvelope.Envelope.VerificationReference
	manifest.Handoff.OperatorReference = publicationEnvelope.Envelope.OperatorReference
	manifest.Handoff.VerificationTimestamp = publicationEnvelope.Envelope.VerificationTimestamp
	manifest.Handoff.PublicationChannel = publicationEnvelope.Envelope.PublicationChannel
	manifest.Handoff.PublicationLocationReference = publicationEnvelope.Envelope.PublicationLocationReference
	manifest.Handoff.DeliveryDestinationReference = publicationEnvelope.Envelope.DeliveryDestinationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationEnvelope", Digest: publicationEnvelopeDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationIndex", Digest: publicationIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationAttestation", Digest: publicationAttestationDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationPackage", Digest: publicationPackageDigest},
		{Kind: "WorkflowRolloutClosureVerifyAttestationIndexManifest", Digest: attestationIndexDigest},
		...
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationHandoffReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.Handoff.VerificationPublicationHandoffReference,
				manifest.Handoff.ReceivedByReference,
				manifest.Handoff.HandoffTimestamp,
				manifest.Handoff.VerificationPublicationEnvelopeReference,
				manifest.Handoff.VerificationPublicationIndexReference,
				manifest.Handoff.VerificationPublicationReference,
				manifest.Handoff.VerificationPackageReference,
				manifest.Handoff.AttestationIndexReference,
				manifest.Handoff.AttestationReference,
				manifest.Handoff.VerificationReference,
				manifest.Handoff.Continuity.AuthorizationID,
				manifest.Handoff.Continuity.TargetDigest,
				manifest.Handoff.BlockerCode,
			}, "|"))),
		},
	}
	return manifest, subjects, nil
...
func buildWorkflowRolloutClosureVerifyPublicationAcknowledgmentManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationAcknowledgmentExportRequest) (workflowRolloutClosureVerifyPublicationAcknowledgmentManifest, []audit.Subject, error) {
	publicationHandoff, publicationHandoffPath, publicationHandoffDigest, err := loadLatestRolloutClosureVerifyPublicationHandoff(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationEnvelope, publicationEnvelopePath, publicationEnvelopeDigest, err := loadLatestRolloutClosureVerifyPublicationEnvelope(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationHandoff.Handoff.HandoffState == "handoff-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-005: latest verification publication handoff is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationHandoff.Handoff.Continuity != publicationEnvelope.Envelope.Continuity ||
		publicationHandoff.Handoff.Continuity != publicationIndex.Index.Continuity ||
		publicationHandoff.Handoff.Continuity != publicationAttestation.Publication.Continuity ||
		publicationHandoff.Handoff.Continuity != publicationPackage.Package.Continuity ||
		publicationHandoff.Handoff.Continuity != attestationIndex.Index.Continuity ||
		publicationHandoff.Handoff.Continuity != attestation.Attestation.Continuity ||
		publicationHandoff.Handoff.Continuity != verifyExport.Export.Continuity ||
		publicationHandoff.Handoff.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-006: latest publication-handoff/envelope/index/attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationHandoff.Handoff.VerificationPublicationEnvelope.Digest != publicationEnvelopeDigest {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-007: latest verification publication handoff does not bind the latest publication envelope digest",
		}
	}
	if publicationHandoff.Handoff.VerificationPublicationIndex.Digest != publicationIndexDigest {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-008: latest verification publication handoff does not bind the latest publication index digest",
		}
	}
	if publicationHandoff.Handoff.VerificationPublicationAttestation.Digest != publicationAttestationDigest {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-009: latest verification publication handoff does not bind the latest publication attestation digest",
		}
	}
	if publicationHandoff.Handoff.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-010: latest verification publication handoff does not bind the latest publication package digest",
		}
	}
	if publicationHandoff.Handoff.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-011: latest verification publication handoff does not bind the latest attestation index digest",
		}
	}
	if publicationHandoff.Handoff.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-012: latest verification publication handoff does not bind the latest verification attestation digest",
		}
	}
	if publicationHandoff.Handoff.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-013: latest verification publication handoff does not bind the latest verification export json digest",
		}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationHandoff.Handoff.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAK-014: latest verification publication handoff does not bind the latest verification export markdown digest",
		}
	}
	acknowledgmentState := "acknowledgment-ready"
	blockerCode := ""
	if publicationHandoff.Handoff.HandoffState == "handoff-blocked" ||
		publicationEnvelope.Envelope.EnvelopeState == "envelope-blocked" ||
		publicationIndex.Index.IndexState == "index-blocked" ||
		publicationAttestation.Publication.PublicationState == "publication-blocked" ||
		publicationPackage.Package.PackageState == "package-blocked" ||
		attestationIndex.Index.IndexState == "index-blocked" ||
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		acknowledgmentState = "acknowledgment-blocked"
		blockerCode = mapValueOrDefault(publicationHandoff.Handoff.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPAK-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationAcknowledgmentManifest{Valid: true}
	manifest.Acknowledgment.WorkspacePath = workspacePath
	manifest.Acknowledgment.VerificationPublicationAcknowledgmentReference = strings.TrimSpace(payload.VerificationPublicationAcknowledgmentReference)
	manifest.Acknowledgment.AcknowledgedByReference = strings.TrimSpace(payload.AcknowledgedByReference)
	manifest.Acknowledgment.AcknowledgmentTimestamp = strings.TrimSpace(payload.AcknowledgmentTimestamp)
	manifest.Acknowledgment.AcknowledgmentState = acknowledgmentState
	manifest.Acknowledgment.BlockerCode = blockerCode
	manifest.Acknowledgment.Continuity = publicationHandoff.Handoff.Continuity
	manifest.Acknowledgment.VerificationPublicationHandoff = workflowClosureArtifact{Path: publicationHandoffPath, Digest: publicationHandoffDigest}
	manifest.Acknowledgment.VerificationPublicationEnvelope = workflowClosureArtifact{Path: publicationEnvelopePath, Digest: publicationEnvelopeDigest}
	manifest.Acknowledgment.VerificationPublicationIndex = workflowClosureArtifact{Path: publicationIndexPath, Digest: publicationIndexDigest}
	manifest.Acknowledgment.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.Acknowledgment.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.Acknowledgment.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.Acknowledgment.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.Acknowledgment.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.Acknowledgment.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.Acknowledgment.VerificationPublicationHandoffReference = publicationHandoff.Handoff.VerificationPublicationHandoffReference
	manifest.Acknowledgment.VerificationPublicationEnvelopeReference = publicationHandoff.Handoff.VerificationPublicationEnvelopeReference
	manifest.Acknowledgment.VerificationPublicationIndexReference = publicationHandoff.Handoff.VerificationPublicationIndexReference
	manifest.Acknowledgment.VerificationPublicationReference = publicationHandoff.Handoff.VerificationPublicationReference
	manifest.Acknowledgment.VerificationPackageReference = publicationHandoff.Handoff.VerificationPackageReference
	manifest.Acknowledgment.AttestationIndexReference = publicationHandoff.Handoff.AttestationIndexReference
	manifest.Acknowledgment.AttestationReference = publicationHandoff.Handoff.AttestationReference
	manifest.Acknowledgment.VerificationReference = publicationHandoff.Handoff.VerificationReference
	manifest.Acknowledgment.OperatorReference = publicationHandoff.Handoff.OperatorReference
	manifest.Acknowledgment.VerificationTimestamp = publicationHandoff.Handoff.VerificationTimestamp
	manifest.Acknowledgment.PublicationChannel = publicationHandoff.Handoff.PublicationChannel
	manifest.Acknowledgment.PublicationLocationReference = publicationHandoff.Handoff.PublicationLocationReference
	manifest.Acknowledgment.DeliveryDestinationReference = publicationHandoff.Handoff.DeliveryDestinationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationHandoff", Digest: publicationHandoffDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationEnvelope", Digest: publicationEnvelopeDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationIndex", Digest: publicationIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationAttestation", Digest: publicationAttestationDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationPackage", Digest: publicationPackageDigest},
		...
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationAcknowledgmentReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.Acknowledgment.VerificationPublicationAcknowledgmentReference,
				manifest.Acknowledgment.AcknowledgedByReference,
				manifest.Acknowledgment.AcknowledgmentTimestamp,
				manifest.Acknowledgment.VerificationPublicationHandoffReference,
				manifest.Acknowledgment.VerificationPublicationEnvelopeReference,
				manifest.Acknowledgment.VerificationPublicationIndexReference,
				manifest.Acknowledgment.VerificationPublicationReference,
				manifest.Acknowledgment.VerificationPackageReference,
				manifest.Acknowledgment.AttestationIndexReference,
				manifest.Acknowledgment.AttestationReference,
				manifest.Acknowledgment.VerificationReference,
				manifest.Acknowledgment.Continuity.AuthorizationID,
				manifest.Acknowledgment.Continuity.TargetDigest,
				manifest.Acknowledgment.BlockerCode,
			}, "|"))),
		},
	}
	return manifest, subjects, nil
...
func buildWorkflowRolloutClosureVerifyPublicationArchiveIndexManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationArchiveIndexExportRequest) (workflowRolloutClosureVerifyPublicationArchiveIndexManifest, []audit.Subject, error) {
	publicationAcknowledgment, publicationAcknowledgmentPath, publicationAcknowledgmentDigest, err := loadLatestRolloutClosureVerifyPublicationAcknowledgment(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationHandoff, publicationHandoffPath, publicationHandoffDigest, err := loadLatestRolloutClosureVerifyPublicationHandoff(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationAcknowledgment.Acknowledgment.AcknowledgmentState == "acknowledgment-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-005: latest verification publication acknowledgment is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationAcknowledgment.Acknowledgment.Continuity != publicationHandoff.Handoff.Continuity ||
		publicationAcknowledgment.Acknowledgment.Continuity != publicationEnvelope.Envelope.Continuity ||
		publicationAcknowledgment.Acknowledgment.Continuity != publicationIndex.Index.Continuity ||
		publicationAcknowledgment.Acknowledgment.Continuity != publicationAttestation.Publication.Continuity ||
		publicationAcknowledgment.Acknowledgment.Continuity != publicationPackage.Package.Continuity ||
		publicationAcknowledgment.Acknowledgment.Continuity != attestationIndex.Index.Continuity ||
		publicationAcknowledgment.Acknowledgment.Continuity != attestation.Attestation.Continuity ||
		publicationAcknowledgment.Acknowledgment.Continuity != verifyExport.Export.Continuity ||
		publicationAcknowledgment.Acknowledgment.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-006: latest publication-acknowledgment/handoff/envelope/index/attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationAcknowledgment.Acknowledgment.VerificationPublicationHandoff.Digest != publicationHandoffDigest {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-007: latest verification publication acknowledgment does not bind the latest publication handoff digest",
		}
	}
	if publicationAcknowledgment.Acknowledgment.VerificationPublicationEnvelope.Digest != publicationEnvelopeDigest {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-008: latest verification publication acknowledgment does not bind the latest publication envelope digest",
		}
	}
	if publicationAcknowledgment.Acknowledgment.VerificationPublicationIndex.Digest != publicationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-009: latest verification publication acknowledgment does not bind the latest publication index digest",
		}
	}
	if publicationAcknowledgment.Acknowledgment.VerificationPublicationAttestation.Digest != publicationAttestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-010: latest verification publication acknowledgment does not bind the latest publication attestation digest",
		}
	}
	if publicationAcknowledgment.Acknowledgment.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-011: latest verification publication acknowledgment does not bind the latest publication package digest",
		}
	}
	if publicationAcknowledgment.Acknowledgment.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-012: latest verification publication acknowledgment does not bind the latest attestation index digest",
		}
	}
	if publicationAcknowledgment.Acknowledgment.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-013: latest verification publication acknowledgment does not bind the latest verification attestation digest",
		}
	}
	if publicationAcknowledgment.Acknowledgment.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-014: latest verification publication acknowledgment does not bind the latest verification export json digest",
		}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationAcknowledgment.Acknowledgment.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationArchiveIndexManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAX-015: latest verification publication acknowledgment does not bind the latest verification export markdown digest",
		}
	}
	archiveIndexState := "archive-index-ready"
	blockerCode := ""
	if publicationAcknowledgment.Acknowledgment.AcknowledgmentState == "acknowledgment-blocked" ||
		publicationHandoff.Handoff.HandoffState == "handoff-blocked" ||
		publicationEnvelope.Envelope.EnvelopeState == "envelope-blocked" ||
		publicationIndex.Index.IndexState == "index-blocked" ||
		publicationAttestation.Publication.PublicationState == "publication-blocked" ||
		publicationPackage.Package.PackageState == "package-blocked" ||
		...
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		archiveIndexState = "archive-index-blocked"
		blockerCode = mapValueOrDefault(publicationAcknowledgment.Acknowledgment.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPAX-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationArchiveIndexManifest{Valid: true}
	manifest.ArchiveIndex.WorkspacePath = workspacePath
	manifest.ArchiveIndex.VerificationPublicationArchiveIndexReference = strings.TrimSpace(payload.VerificationPublicationArchiveIndexReference)
	manifest.ArchiveIndex.IndexedByReference = strings.TrimSpace(payload.IndexedByReference)
	manifest.ArchiveIndex.IndexedTimestamp = strings.TrimSpace(payload.IndexedTimestamp)
	manifest.ArchiveIndex.ArchiveIndexState = archiveIndexState
	manifest.ArchiveIndex.BlockerCode = blockerCode
	manifest.ArchiveIndex.Continuity = publicationAcknowledgment.Acknowledgment.Continuity
	manifest.ArchiveIndex.VerificationPublicationAcknowledgment = workflowClosureArtifact{Path: publicationAcknowledgmentPath, Digest: publicationAcknowledgmentDigest}
	manifest.ArchiveIndex.VerificationPublicationHandoff = workflowClosureArtifact{Path: publicationHandoffPath, Digest: publicationHandoffDigest}
	manifest.ArchiveIndex.VerificationPublicationEnvelope = workflowClosureArtifact{Path: publicationEnvelopePath, Digest: publicationEnvelopeDigest}
	manifest.ArchiveIndex.VerificationPublicationIndex = workflowClosureArtifact{Path: publicationIndexPath, Digest: publicationIndexDigest}
	manifest.ArchiveIndex.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.ArchiveIndex.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.ArchiveIndex.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.ArchiveIndex.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.ArchiveIndex.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.ArchiveIndex.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.ArchiveIndex.VerificationPublicationAcknowledgmentReference = publicationAcknowledgment.Acknowledgment.VerificationPublicationAcknowledgmentReference
	manifest.ArchiveIndex.VerificationPublicationHandoffReference = publicationAcknowledgment.Acknowledgment.VerificationPublicationHandoffReference
	manifest.ArchiveIndex.VerificationPublicationEnvelopeReference = publicationAcknowledgment.Acknowledgment.VerificationPublicationEnvelopeReference
	manifest.ArchiveIndex.VerificationPublicationIndexReference = publicationAcknowledgment.Acknowledgment.VerificationPublicationIndexReference
	manifest.ArchiveIndex.VerificationPublicationReference = publicationAcknowledgment.Acknowledgment.VerificationPublicationReference
	manifest.ArchiveIndex.VerificationPackageReference = publicationAcknowledgment.Acknowledgment.VerificationPackageReference
	manifest.ArchiveIndex.AttestationIndexReference = publicationAcknowledgment.Acknowledgment.AttestationIndexReference
	manifest.ArchiveIndex.AttestationReference = publicationAcknowledgment.Acknowledgment.AttestationReference
	manifest.ArchiveIndex.VerificationReference = publicationAcknowledgment.Acknowledgment.VerificationReference
	manifest.ArchiveIndex.OperatorReference = publicationAcknowledgment.Acknowledgment.OperatorReference
	manifest.ArchiveIndex.VerificationTimestamp = publicationAcknowledgment.Acknowledgment.VerificationTimestamp
	manifest.ArchiveIndex.PublicationChannel = publicationAcknowledgment.Acknowledgment.PublicationChannel
	manifest.ArchiveIndex.PublicationLocationReference = publicationAcknowledgment.Acknowledgment.PublicationLocationReference
	manifest.ArchiveIndex.DeliveryDestinationReference = publicationAcknowledgment.Acknowledgment.DeliveryDestinationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationAcknowledgment", Digest: publicationAcknowledgmentDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationHandoff", Digest: publicationHandoffDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationEnvelope", Digest: publicationEnvelopeDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationIndex", Digest: publicationIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationAttestation", Digest: publicationAttestationDigest},
		...
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationArchiveIndexReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.ArchiveIndex.VerificationPublicationArchiveIndexReference,
				manifest.ArchiveIndex.IndexedByReference,
				manifest.ArchiveIndex.IndexedTimestamp,
				manifest.ArchiveIndex.VerificationPublicationAcknowledgmentReference,
				manifest.ArchiveIndex.VerificationPublicationHandoffReference,
				manifest.ArchiveIndex.VerificationPublicationEnvelopeReference,
				manifest.ArchiveIndex.VerificationPublicationIndexReference,
				manifest.ArchiveIndex.VerificationPublicationReference,
				manifest.ArchiveIndex.VerificationPackageReference,
				manifest.ArchiveIndex.AttestationIndexReference,
				manifest.ArchiveIndex.AttestationReference,
				manifest.ArchiveIndex.VerificationReference,
				manifest.ArchiveIndex.Continuity.AuthorizationID,
				manifest.ArchiveIndex.Continuity.TargetDigest,
				manifest.ArchiveIndex.BlockerCode,
			}, "|"))),
		},
	}
	return manifest, subjects, nil
...
func buildWorkflowRolloutClosureVerifyPublicationArchivePackageManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationArchivePackageExportRequest) (workflowRolloutClosureVerifyPublicationArchivePackageManifest, []audit.Subject, error) {
	publicationArchiveIndex, publicationArchiveIndexPath, publicationArchiveIndexDigest, err := loadLatestRolloutClosureVerifyPublicationArchiveIndex(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationAcknowledgment, publicationAcknowledgmentPath, publicationAcknowledgmentDigest, err := loadLatestRolloutClosureVerifyPublicationAcknowledgment(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.ArchiveIndexState == "archive-index-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-005: latest verification publication archive index is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationArchiveIndex.ArchiveIndex.Continuity != publicationAcknowledgment.Acknowledgment.Continuity ||
		publicationArchiveIndex.ArchiveIndex.Continuity != publicationHandoff.Handoff.Continuity ||
		publicationArchiveIndex.ArchiveIndex.Continuity != publicationEnvelope.Envelope.Continuity ||
		publicationArchiveIndex.ArchiveIndex.Continuity != publicationIndex.Index.Continuity ||
		publicationArchiveIndex.ArchiveIndex.Continuity != publicationAttestation.Publication.Continuity ||
		publicationArchiveIndex.ArchiveIndex.Continuity != publicationPackage.Package.Continuity ||
		publicationArchiveIndex.ArchiveIndex.Continuity != attestationIndex.Index.Continuity ||
		publicationArchiveIndex.ArchiveIndex.Continuity != attestation.Attestation.Continuity ||
		publicationArchiveIndex.ArchiveIndex.Continuity != verifyExport.Export.Continuity ||
		publicationArchiveIndex.ArchiveIndex.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-006: latest publication-archive-index/acknowledgment/handoff/envelope/index/attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.VerificationPublicationAcknowledgment.Digest != publicationAcknowledgmentDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-007: latest verification publication archive index does not bind the latest publication acknowledgment digest",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.VerificationPublicationHandoff.Digest != publicationHandoffDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-008: latest verification publication archive index does not bind the latest publication handoff digest",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.VerificationPublicationEnvelope.Digest != publicationEnvelopeDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-009: latest verification publication archive index does not bind the latest publication envelope digest",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.VerificationPublicationIndex.Digest != publicationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-010: latest verification publication archive index does not bind the latest publication index digest",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.VerificationPublicationAttestation.Digest != publicationAttestationDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-011: latest verification publication archive index does not bind the latest publication attestation digest",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-012: latest verification publication archive index does not bind the latest publication package digest",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-013: latest verification publication archive index does not bind the latest attestation index digest",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-014: latest verification publication archive index does not bind the latest verification attestation digest",
		}
	}
	if publicationArchiveIndex.ArchiveIndex.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-015: latest verification publication archive index does not bind the latest verification export json digest",
		}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationArchiveIndex.ArchiveIndex.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationArchivePackageManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAP-016: latest verification publication archive index does not bind the latest verification export markdown digest",
		}
	}
	archivePackageState := "archive-package-ready"
	blockerCode := ""
	if publicationArchiveIndex.ArchiveIndex.ArchiveIndexState == "archive-index-blocked" ||
		publicationAcknowledgment.Acknowledgment.AcknowledgmentState == "acknowledgment-blocked" ||
		publicationHandoff.Handoff.HandoffState == "handoff-blocked" ||
		publicationEnvelope.Envelope.EnvelopeState == "envelope-blocked" ||
		publicationIndex.Index.IndexState == "index-blocked" ||
		publicationAttestation.Publication.PublicationState == "publication-blocked" ||
		...
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		archivePackageState = "archive-package-blocked"
		blockerCode = mapValueOrDefault(publicationArchiveIndex.ArchiveIndex.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPAP-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationArchivePackageManifest{Valid: true}
	manifest.ArchivePackage.WorkspacePath = workspacePath
	manifest.ArchivePackage.VerificationPublicationArchivePackageReference = strings.TrimSpace(payload.VerificationPublicationArchivePackageReference)
	manifest.ArchivePackage.PackagedByReference = strings.TrimSpace(payload.PackagedByReference)
	manifest.ArchivePackage.PackagedTimestamp = strings.TrimSpace(payload.PackagedTimestamp)
	manifest.ArchivePackage.ArchivePackageState = archivePackageState
	manifest.ArchivePackage.BlockerCode = blockerCode
	manifest.ArchivePackage.Continuity = publicationArchiveIndex.ArchiveIndex.Continuity
	manifest.ArchivePackage.VerificationPublicationArchiveIndex = workflowClosureArtifact{Path: publicationArchiveIndexPath, Digest: publicationArchiveIndexDigest}
	manifest.ArchivePackage.VerificationPublicationAcknowledgment = workflowClosureArtifact{Path: publicationAcknowledgmentPath, Digest: publicationAcknowledgmentDigest}
	manifest.ArchivePackage.VerificationPublicationHandoff = workflowClosureArtifact{Path: publicationHandoffPath, Digest: publicationHandoffDigest}
	manifest.ArchivePackage.VerificationPublicationEnvelope = workflowClosureArtifact{Path: publicationEnvelopePath, Digest: publicationEnvelopeDigest}
	manifest.ArchivePackage.VerificationPublicationIndex = workflowClosureArtifact{Path: publicationIndexPath, Digest: publicationIndexDigest}
	manifest.ArchivePackage.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.ArchivePackage.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.ArchivePackage.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.ArchivePackage.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.ArchivePackage.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.ArchivePackage.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.ArchivePackage.VerificationPublicationArchiveIndexReference = publicationArchiveIndex.ArchiveIndex.VerificationPublicationArchiveIndexReference
	manifest.ArchivePackage.VerificationPublicationAcknowledgmentReference = publicationArchiveIndex.ArchiveIndex.VerificationPublicationAcknowledgmentReference
	manifest.ArchivePackage.VerificationPublicationHandoffReference = publicationArchiveIndex.ArchiveIndex.VerificationPublicationHandoffReference
	manifest.ArchivePackage.VerificationPublicationEnvelopeReference = publicationArchiveIndex.ArchiveIndex.VerificationPublicationEnvelopeReference
	manifest.ArchivePackage.VerificationPublicationIndexReference = publicationArchiveIndex.ArchiveIndex.VerificationPublicationIndexReference
	manifest.ArchivePackage.VerificationPublicationReference = publicationArchiveIndex.ArchiveIndex.VerificationPublicationReference
	manifest.ArchivePackage.VerificationPackageReference = publicationArchiveIndex.ArchiveIndex.VerificationPackageReference
	manifest.ArchivePackage.AttestationIndexReference = publicationArchiveIndex.ArchiveIndex.AttestationIndexReference
	manifest.ArchivePackage.AttestationReference = publicationArchiveIndex.ArchiveIndex.AttestationReference
	manifest.ArchivePackage.VerificationReference = publicationArchiveIndex.ArchiveIndex.VerificationReference
	manifest.ArchivePackage.OperatorReference = publicationArchiveIndex.ArchiveIndex.OperatorReference
	manifest.ArchivePackage.VerificationTimestamp = publicationArchiveIndex.ArchiveIndex.VerificationTimestamp
	manifest.ArchivePackage.PublicationChannel = publicationArchiveIndex.ArchiveIndex.PublicationChannel
	manifest.ArchivePackage.PublicationLocationReference = publicationArchiveIndex.ArchiveIndex.PublicationLocationReference
	manifest.ArchivePackage.DeliveryDestinationReference = publicationArchiveIndex.ArchiveIndex.DeliveryDestinationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveIndex", Digest: publicationArchiveIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationAcknowledgment", Digest: publicationAcknowledgmentDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationHandoff", Digest: publicationHandoffDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationEnvelope", Digest: publicationEnvelopeDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationIndex", Digest: publicationIndexDigest},
		...
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationArchivePackageReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.ArchivePackage.VerificationPublicationArchivePackageReference,
				manifest.ArchivePackage.PackagedByReference,
				manifest.ArchivePackage.PackagedTimestamp,
				manifest.ArchivePackage.VerificationPublicationArchiveIndexReference,
				manifest.ArchivePackage.VerificationPublicationAcknowledgmentReference,
				manifest.ArchivePackage.VerificationPublicationHandoffReference,
				manifest.ArchivePackage.VerificationPublicationEnvelopeReference,
				manifest.ArchivePackage.VerificationPublicationIndexReference,
				manifest.ArchivePackage.VerificationPublicationReference,
				manifest.ArchivePackage.VerificationPackageReference,
				manifest.ArchivePackage.AttestationIndexReference,
				manifest.ArchivePackage.AttestationReference,
				manifest.ArchivePackage.VerificationReference,
				manifest.ArchivePackage.Continuity.AuthorizationID,
				manifest.ArchivePackage.Continuity.TargetDigest,
				manifest.ArchivePackage.BlockerCode,
			}, "|"))),
		},
	}
	return manifest, subjects, nil
...
func buildWorkflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationArchiveEnvelopeExportRequest) (workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest, []audit.Subject, error) {
	publicationArchivePackage, publicationArchivePackagePath, publicationArchivePackageDigest, err := loadLatestRolloutClosureVerifyPublicationArchivePackage(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationArchiveIndex, publicationArchiveIndexPath, publicationArchiveIndexDigest, err := loadLatestRolloutClosureVerifyPublicationArchiveIndex(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationArchivePackage.ArchivePackage.ArchivePackageState == "archive-package-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-005: latest verification publication archive package is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationArchivePackage.ArchivePackage.Continuity != publicationArchiveIndex.ArchiveIndex.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != publicationAcknowledgment.Acknowledgment.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != publicationHandoff.Handoff.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != publicationEnvelope.Envelope.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != publicationIndex.Index.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != publicationAttestation.Publication.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != publicationPackage.Package.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != attestationIndex.Index.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != attestation.Attestation.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != verifyExport.Export.Continuity ||
		publicationArchivePackage.ArchivePackage.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-006: latest publication-archive-package/archive-index/acknowledgment/handoff/envelope/index/attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationPublicationArchiveIndex.Digest != publicationArchiveIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-007: latest verification publication archive package does not bind the latest publication archive index digest",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationPublicationAcknowledgment.Digest != publicationAcknowledgmentDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-008: latest verification publication archive package does not bind the latest publication acknowledgment digest",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationPublicationHandoff.Digest != publicationHandoffDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-009: latest verification publication archive package does not bind the latest publication handoff digest",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationPublicationEnvelope.Digest != publicationEnvelopeDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-010: latest verification publication archive package does not bind the latest publication envelope digest",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationPublicationIndex.Digest != publicationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-011: latest verification publication archive package does not bind the latest publication index digest",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationPublicationAttestation.Digest != publicationAttestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-012: latest verification publication archive package does not bind the latest publication attestation digest",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-013: latest verification publication archive package does not bind the latest publication package digest",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-014: latest verification publication archive package does not bind the latest attestation index digest",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-015: latest verification publication archive package does not bind the latest verification attestation digest",
		}
	}
	if publicationArchivePackage.ArchivePackage.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-016: latest verification publication archive package does not bind the latest verification export json digest",
		}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationArchivePackage.ArchivePackage.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAE-017: latest verification publication archive package does not bind the latest verification export markdown digest",
		}
	}
	archiveEnvelopeState := "archive-envelope-ready"
	blockerCode := ""
	if publicationArchivePackage.ArchivePackage.ArchivePackageState == "archive-package-blocked" ||
		publicationArchiveIndex.ArchiveIndex.ArchiveIndexState == "archive-index-blocked" ||
		publicationAcknowledgment.Acknowledgment.AcknowledgmentState == "acknowledgment-blocked" ||
		publicationHandoff.Handoff.HandoffState == "handoff-blocked" ||
		publicationEnvelope.Envelope.EnvelopeState == "envelope-blocked" ||
		publicationIndex.Index.IndexState == "index-blocked" ||
		...
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		archiveEnvelopeState = "archive-envelope-blocked"
		blockerCode = mapValueOrDefault(publicationArchivePackage.ArchivePackage.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPAE-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationArchiveEnvelopeManifest{Valid: true}
	manifest.ArchiveEnvelope.WorkspacePath = workspacePath
	manifest.ArchiveEnvelope.VerificationPublicationArchiveEnvelopeReference = strings.TrimSpace(payload.VerificationPublicationArchiveEnvelopeReference)
	manifest.ArchiveEnvelope.DeliveredByReference = strings.TrimSpace(payload.DeliveredByReference)
	manifest.ArchiveEnvelope.DeliveryTimestamp = strings.TrimSpace(payload.DeliveryTimestamp)
	manifest.ArchiveEnvelope.DeliveryDestinationReference = strings.TrimSpace(payload.DeliveryDestinationReference)
	manifest.ArchiveEnvelope.ArchiveEnvelopeState = archiveEnvelopeState
	manifest.ArchiveEnvelope.BlockerCode = blockerCode
	manifest.ArchiveEnvelope.Continuity = publicationArchivePackage.ArchivePackage.Continuity
	manifest.ArchiveEnvelope.VerificationPublicationArchivePackage = workflowClosureArtifact{Path: publicationArchivePackagePath, Digest: publicationArchivePackageDigest}
	manifest.ArchiveEnvelope.VerificationPublicationArchiveIndex = workflowClosureArtifact{Path: publicationArchiveIndexPath, Digest: publicationArchiveIndexDigest}
	manifest.ArchiveEnvelope.VerificationPublicationAcknowledgment = workflowClosureArtifact{Path: publicationAcknowledgmentPath, Digest: publicationAcknowledgmentDigest}
	manifest.ArchiveEnvelope.VerificationPublicationHandoff = workflowClosureArtifact{Path: publicationHandoffPath, Digest: publicationHandoffDigest}
	manifest.ArchiveEnvelope.VerificationPublicationEnvelope = workflowClosureArtifact{Path: publicationEnvelopePath, Digest: publicationEnvelopeDigest}
	manifest.ArchiveEnvelope.VerificationPublicationIndex = workflowClosureArtifact{Path: publicationIndexPath, Digest: publicationIndexDigest}
	manifest.ArchiveEnvelope.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.ArchiveEnvelope.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.ArchiveEnvelope.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.ArchiveEnvelope.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.ArchiveEnvelope.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.ArchiveEnvelope.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.ArchiveEnvelope.VerificationPublicationArchivePackageReference = publicationArchivePackage.ArchivePackage.VerificationPublicationArchivePackageReference
	manifest.ArchiveEnvelope.VerificationPublicationArchiveIndexReference = publicationArchivePackage.ArchivePackage.VerificationPublicationArchiveIndexReference
	manifest.ArchiveEnvelope.VerificationPublicationAcknowledgmentReference = publicationArchivePackage.ArchivePackage.VerificationPublicationAcknowledgmentReference
	manifest.ArchiveEnvelope.VerificationPublicationHandoffReference = publicationArchivePackage.ArchivePackage.VerificationPublicationHandoffReference
	manifest.ArchiveEnvelope.VerificationPublicationEnvelopeReference = publicationArchivePackage.ArchivePackage.VerificationPublicationEnvelopeReference
	manifest.ArchiveEnvelope.VerificationPublicationIndexReference = publicationArchivePackage.ArchivePackage.VerificationPublicationIndexReference
	manifest.ArchiveEnvelope.VerificationPublicationReference = publicationArchivePackage.ArchivePackage.VerificationPublicationReference
	manifest.ArchiveEnvelope.VerificationPackageReference = publicationArchivePackage.ArchivePackage.VerificationPackageReference
	manifest.ArchiveEnvelope.AttestationIndexReference = publicationArchivePackage.ArchivePackage.AttestationIndexReference
	manifest.ArchiveEnvelope.AttestationReference = publicationArchivePackage.ArchivePackage.AttestationReference
	manifest.ArchiveEnvelope.VerificationReference = publicationArchivePackage.ArchivePackage.VerificationReference
	manifest.ArchiveEnvelope.OperatorReference = publicationArchivePackage.ArchivePackage.OperatorReference
	manifest.ArchiveEnvelope.VerificationTimestamp = publicationArchivePackage.ArchivePackage.VerificationTimestamp
	manifest.ArchiveEnvelope.PublicationChannel = publicationArchivePackage.ArchivePackage.PublicationChannel
	manifest.ArchiveEnvelope.PublicationLocationReference = publicationArchivePackage.ArchivePackage.PublicationLocationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchivePackage", Digest: publicationArchivePackageDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveIndex", Digest: publicationArchiveIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationAcknowledgment", Digest: publicationAcknowledgmentDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationHandoff", Digest: publicationHandoffDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationEnvelope", Digest: publicationEnvelopeDigest},
		...
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationArchiveEnvelopeReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.ArchiveEnvelope.VerificationPublicationArchiveEnvelopeReference,
				manifest.ArchiveEnvelope.DeliveredByReference,
				manifest.ArchiveEnvelope.DeliveryTimestamp,
				manifest.ArchiveEnvelope.DeliveryDestinationReference,
				manifest.ArchiveEnvelope.VerificationPublicationArchivePackageReference,
				manifest.ArchiveEnvelope.VerificationPublicationArchiveIndexReference,
				manifest.ArchiveEnvelope.VerificationPublicationAcknowledgmentReference,
				manifest.ArchiveEnvelope.VerificationPublicationHandoffReference,
				manifest.ArchiveEnvelope.VerificationPublicationEnvelopeReference,
				manifest.ArchiveEnvelope.VerificationPublicationIndexReference,
				manifest.ArchiveEnvelope.VerificationPublicationReference,
				manifest.ArchiveEnvelope.VerificationPackageReference,
				manifest.ArchiveEnvelope.AttestationIndexReference,
				manifest.ArchiveEnvelope.AttestationReference,
				manifest.ArchiveEnvelope.VerificationReference,
				manifest.ArchiveEnvelope.Continuity.AuthorizationID,
				manifest.ArchiveEnvelope.Continuity.TargetDigest,
				manifest.ArchiveEnvelope.BlockerCode,
			}, "|"))),
		},
	}
	return manifest, subjects, nil
...
func buildWorkflowRolloutClosureVerifyPublicationArchiveHandoffManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationArchiveHandoffExportRequest) (workflowRolloutClosureVerifyPublicationArchiveHandoffManifest, []audit.Subject, error) {
	publicationArchiveEnvelope, publicationArchiveEnvelopePath, publicationArchiveEnvelopeDigest, err := loadLatestRolloutClosureVerifyPublicationArchiveEnvelope(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationArchivePackage, publicationArchivePackagePath, publicationArchivePackageDigest, err := loadLatestRolloutClosureVerifyPublicationArchivePackage(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAH-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAH-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.ArchiveEnvelopeState == "archive-envelope-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAH-005: latest verification publication archive envelope is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationArchiveEnvelope.ArchiveEnvelope.Continuity != publicationArchivePackage.ArchivePackage.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != publicationArchiveIndex.ArchiveIndex.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != publicationAcknowledgment.Acknowledgment.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != publicationHandoff.Handoff.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != publicationEnvelope.Envelope.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != publicationIndex.Index.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != publicationAttestation.Publication.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != publicationPackage.Package.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != attestationIndex.Index.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != attestation.Attestation.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != verifyExport.Export.Continuity ||
		publicationArchiveEnvelope.ArchiveEnvelope.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAH-006: latest publication-archive-envelope/archive-package/archive-index/acknowledgment/handoff/envelope/index/attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationArchivePackage.Digest != publicationArchivePackageDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-007: latest verification publication archive envelope does not bind the latest publication archive package digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationArchiveIndex.Digest != publicationArchiveIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-008: latest verification publication archive envelope does not bind the latest publication archive index digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationAcknowledgment.Digest != publicationAcknowledgmentDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-009: latest verification publication archive envelope does not bind the latest publication acknowledgment digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationHandoff.Digest != publicationHandoffDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-010: latest verification publication archive envelope does not bind the latest publication handoff digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationEnvelope.Digest != publicationEnvelopeDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-011: latest verification publication archive envelope does not bind the latest publication envelope digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationIndex.Digest != publicationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-012: latest verification publication archive envelope does not bind the latest publication index digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationAttestation.Digest != publicationAttestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-013: latest verification publication archive envelope does not bind the latest publication attestation digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-014: latest verification publication archive envelope does not bind the latest publication package digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-015: latest verification publication archive envelope does not bind the latest attestation index digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-016: latest verification publication archive envelope does not bind the latest verification attestation digest"}
	}
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-017: latest verification publication archive envelope does not bind the latest verification export json digest"}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationArchiveEnvelope.ArchiveEnvelope.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAH-018: latest verification publication archive envelope does not bind the latest verification export markdown digest"}
	}
	archiveHandoffState := "archive-handoff-ready"
	blockerCode := ""
	if publicationArchiveEnvelope.ArchiveEnvelope.ArchiveEnvelopeState == "archive-envelope-blocked" ||
		publicationArchivePackage.ArchivePackage.ArchivePackageState == "archive-package-blocked" ||
		publicationArchiveIndex.ArchiveIndex.ArchiveIndexState == "archive-index-blocked" ||
		publicationAcknowledgment.Acknowledgment.AcknowledgmentState == "acknowledgment-blocked" ||
		publicationHandoff.Handoff.HandoffState == "handoff-blocked" ||
		publicationEnvelope.Envelope.EnvelopeState == "envelope-blocked" ||
		...
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		archiveHandoffState = "archive-handoff-blocked"
		blockerCode = mapValueOrDefault(publicationArchiveEnvelope.ArchiveEnvelope.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPAH-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationArchiveHandoffManifest{Valid: true}
	manifest.ArchiveHandoff.WorkspacePath = workspacePath
	manifest.ArchiveHandoff.VerificationPublicationArchiveHandoffReference = strings.TrimSpace(payload.VerificationPublicationArchiveHandoffReference)
	manifest.ArchiveHandoff.ReceivedByReference = strings.TrimSpace(payload.ReceivedByReference)
	manifest.ArchiveHandoff.HandoffTimestamp = strings.TrimSpace(payload.HandoffTimestamp)
	manifest.ArchiveHandoff.ArchiveHandoffState = archiveHandoffState
	manifest.ArchiveHandoff.BlockerCode = blockerCode
	manifest.ArchiveHandoff.Continuity = publicationArchiveEnvelope.ArchiveEnvelope.Continuity
	manifest.ArchiveHandoff.VerificationPublicationArchiveEnvelope = workflowClosureArtifact{Path: publicationArchiveEnvelopePath, Digest: publicationArchiveEnvelopeDigest}
	manifest.ArchiveHandoff.VerificationPublicationArchivePackage = workflowClosureArtifact{Path: publicationArchivePackagePath, Digest: publicationArchivePackageDigest}
	manifest.ArchiveHandoff.VerificationPublicationArchiveIndex = workflowClosureArtifact{Path: publicationArchiveIndexPath, Digest: publicationArchiveIndexDigest}
	manifest.ArchiveHandoff.VerificationPublicationAcknowledgment = workflowClosureArtifact{Path: publicationAcknowledgmentPath, Digest: publicationAcknowledgmentDigest}
	manifest.ArchiveHandoff.VerificationPublicationHandoff = workflowClosureArtifact{Path: publicationHandoffPath, Digest: publicationHandoffDigest}
	manifest.ArchiveHandoff.VerificationPublicationEnvelope = workflowClosureArtifact{Path: publicationEnvelopePath, Digest: publicationEnvelopeDigest}
	manifest.ArchiveHandoff.VerificationPublicationIndex = workflowClosureArtifact{Path: publicationIndexPath, Digest: publicationIndexDigest}
	manifest.ArchiveHandoff.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.ArchiveHandoff.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.ArchiveHandoff.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.ArchiveHandoff.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.ArchiveHandoff.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.ArchiveHandoff.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.ArchiveHandoff.VerificationPublicationArchiveEnvelopeReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationArchiveEnvelopeReference
	manifest.ArchiveHandoff.VerificationPublicationArchivePackageReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationArchivePackageReference
	manifest.ArchiveHandoff.VerificationPublicationArchiveIndexReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationArchiveIndexReference
	manifest.ArchiveHandoff.VerificationPublicationAcknowledgmentReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationAcknowledgmentReference
	manifest.ArchiveHandoff.VerificationPublicationHandoffReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationHandoffReference
	manifest.ArchiveHandoff.VerificationPublicationEnvelopeReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationEnvelopeReference
	manifest.ArchiveHandoff.VerificationPublicationIndexReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationIndexReference
	manifest.ArchiveHandoff.VerificationPublicationReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationPublicationReference
	manifest.ArchiveHandoff.VerificationPackageReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationPackageReference
	manifest.ArchiveHandoff.AttestationIndexReference = publicationArchiveEnvelope.ArchiveEnvelope.AttestationIndexReference
	manifest.ArchiveHandoff.AttestationReference = publicationArchiveEnvelope.ArchiveEnvelope.AttestationReference
	manifest.ArchiveHandoff.VerificationReference = publicationArchiveEnvelope.ArchiveEnvelope.VerificationReference
	manifest.ArchiveHandoff.OperatorReference = publicationArchiveEnvelope.ArchiveEnvelope.OperatorReference
	manifest.ArchiveHandoff.VerificationTimestamp = publicationArchiveEnvelope.ArchiveEnvelope.VerificationTimestamp
	manifest.ArchiveHandoff.PublicationChannel = publicationArchiveEnvelope.ArchiveEnvelope.PublicationChannel
	manifest.ArchiveHandoff.PublicationLocationReference = publicationArchiveEnvelope.ArchiveEnvelope.PublicationLocationReference
	manifest.ArchiveHandoff.DeliveryDestinationReference = publicationArchiveEnvelope.ArchiveEnvelope.DeliveryDestinationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveEnvelope", Digest: publicationArchiveEnvelopeDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchivePackage", Digest: publicationArchivePackageDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveIndex", Digest: publicationArchiveIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationAcknowledgment", Digest: publicationAcknowledgmentDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationHandoff", Digest: publicationHandoffDigest},
		...
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationArchiveHandoffReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.ArchiveHandoff.VerificationPublicationArchiveHandoffReference,
				manifest.ArchiveHandoff.ReceivedByReference,
				manifest.ArchiveHandoff.HandoffTimestamp,
				manifest.ArchiveHandoff.VerificationPublicationArchiveEnvelopeReference,
				manifest.ArchiveHandoff.VerificationPublicationArchivePackageReference,
				manifest.ArchiveHandoff.VerificationPublicationArchiveIndexReference,
				manifest.ArchiveHandoff.VerificationPublicationAcknowledgmentReference,
				manifest.ArchiveHandoff.VerificationPublicationHandoffReference,
				manifest.ArchiveHandoff.VerificationPublicationEnvelopeReference,
				manifest.ArchiveHandoff.VerificationPublicationIndexReference,
				manifest.ArchiveHandoff.VerificationPublicationReference,
				manifest.ArchiveHandoff.VerificationPackageReference,
				manifest.ArchiveHandoff.AttestationIndexReference,
				manifest.ArchiveHandoff.AttestationReference,
				manifest.ArchiveHandoff.VerificationReference,
				manifest.ArchiveHandoff.Continuity.AuthorizationID,
				manifest.ArchiveHandoff.Continuity.TargetDigest,
				manifest.ArchiveHandoff.BlockerCode,
			}, "|"))),
		},
	}
	return manifest, subjects, nil
...
func buildWorkflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentExportRequest) (workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest, []audit.Subject, error) {
	publicationArchiveHandoff, publicationArchiveHandoffPath, publicationArchiveHandoffDigest, err := loadLatestRolloutClosureVerifyPublicationArchiveHandoff(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationArchiveEnvelope, publicationArchiveEnvelopePath, publicationArchiveEnvelopeDigest, err := loadLatestRolloutClosureVerifyPublicationArchiveEnvelope(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAA-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAA-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationArchiveHandoff.ArchiveHandoff.ArchiveHandoffState == "archive-handoff-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAA-005: latest verification publication archive handoff is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationArchiveHandoff.ArchiveHandoff.Continuity != publicationArchiveEnvelope.ArchiveEnvelope.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != publicationArchivePackage.ArchivePackage.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != publicationArchiveIndex.ArchiveIndex.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != publicationAcknowledgment.Acknowledgment.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != publicationHandoff.Handoff.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != publicationEnvelope.Envelope.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != publicationIndex.Index.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != publicationAttestation.Publication.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != publicationPackage.Package.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != attestationIndex.Index.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != attestation.Attestation.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != verifyExport.Export.Continuity ||
		publicationArchiveHandoff.ArchiveHandoff.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAA-006: latest publication-archive-handoff/archive-envelope/archive-package/archive-index/acknowledgment/handoff/envelope/index/attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationArchiveEnvelope.Digest != publicationArchiveEnvelopeDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-007: latest verification publication archive handoff does not bind the latest publication archive envelope digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationArchivePackage.Digest != publicationArchivePackageDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-008: latest verification publication archive handoff does not bind the latest publication archive package digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationArchiveIndex.Digest != publicationArchiveIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-009: latest verification publication archive handoff does not bind the latest publication archive index digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationAcknowledgment.Digest != publicationAcknowledgmentDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-010: latest verification publication archive handoff does not bind the latest publication acknowledgment digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationHandoff.Digest != publicationHandoffDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-011: latest verification publication archive handoff does not bind the latest publication handoff digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationEnvelope.Digest != publicationEnvelopeDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-012: latest verification publication archive handoff does not bind the latest publication envelope digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationIndex.Digest != publicationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-013: latest verification publication archive handoff does not bind the latest publication index digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationAttestation.Digest != publicationAttestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-014: latest verification publication archive handoff does not bind the latest publication attestation digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-015: latest verification publication archive handoff does not bind the latest publication package digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-016: latest verification publication archive handoff does not bind the latest attestation index digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-017: latest verification publication archive handoff does not bind the latest verification attestation digest"}
	}
	if publicationArchiveHandoff.ArchiveHandoff.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-018: latest verification publication archive handoff does not bind the latest verification export json digest"}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationArchiveHandoff.ArchiveHandoff.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAA-019: latest verification publication archive handoff does not bind the latest verification export markdown digest"}
	}
	archiveAcknowledgmentState := "archive-acknowledgment-ready"
	blockerCode := ""
	if publicationArchiveHandoff.ArchiveHandoff.ArchiveHandoffState == "archive-handoff-blocked" ||
		publicationArchiveEnvelope.ArchiveEnvelope.ArchiveEnvelopeState == "archive-envelope-blocked" ||
		publicationArchivePackage.ArchivePackage.ArchivePackageState == "archive-package-blocked" ||
		publicationArchiveIndex.ArchiveIndex.ArchiveIndexState == "archive-index-blocked" ||
		publicationAcknowledgment.Acknowledgment.AcknowledgmentState == "acknowledgment-blocked" ||
		publicationHandoff.Handoff.HandoffState == "handoff-blocked" ||
		...
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		archiveAcknowledgmentState = "archive-acknowledgment-blocked"
		blockerCode = mapValueOrDefault(publicationArchiveHandoff.ArchiveHandoff.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPAA-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationArchiveAcknowledgmentManifest{Valid: true}
	manifest.ArchiveAcknowledgment.WorkspacePath = workspacePath
	manifest.ArchiveAcknowledgment.VerificationPublicationArchiveAcknowledgmentReference = strings.TrimSpace(payload.VerificationPublicationArchiveAcknowledgmentReference)
	manifest.ArchiveAcknowledgment.AcknowledgedByReference = strings.TrimSpace(payload.AcknowledgedByReference)
	manifest.ArchiveAcknowledgment.AcknowledgmentTimestamp = strings.TrimSpace(payload.AcknowledgmentTimestamp)
	manifest.ArchiveAcknowledgment.ArchiveAcknowledgmentState = archiveAcknowledgmentState
	manifest.ArchiveAcknowledgment.BlockerCode = blockerCode
	manifest.ArchiveAcknowledgment.Continuity = publicationArchiveHandoff.ArchiveHandoff.Continuity
	manifest.ArchiveAcknowledgment.VerificationPublicationArchiveHandoff = workflowClosureArtifact{Path: publicationArchiveHandoffPath, Digest: publicationArchiveHandoffDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationArchiveEnvelope = workflowClosureArtifact{Path: publicationArchiveEnvelopePath, Digest: publicationArchiveEnvelopeDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationArchivePackage = workflowClosureArtifact{Path: publicationArchivePackagePath, Digest: publicationArchivePackageDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationArchiveIndex = workflowClosureArtifact{Path: publicationArchiveIndexPath, Digest: publicationArchiveIndexDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationAcknowledgment = workflowClosureArtifact{Path: publicationAcknowledgmentPath, Digest: publicationAcknowledgmentDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationHandoff = workflowClosureArtifact{Path: publicationHandoffPath, Digest: publicationHandoffDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationEnvelope = workflowClosureArtifact{Path: publicationEnvelopePath, Digest: publicationEnvelopeDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationIndex = workflowClosureArtifact{Path: publicationIndexPath, Digest: publicationIndexDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.ArchiveAcknowledgment.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.ArchiveAcknowledgment.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.ArchiveAcknowledgment.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.ArchiveAcknowledgment.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.ArchiveAcknowledgment.VerificationPublicationArchiveHandoffReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationArchiveHandoffReference
	manifest.ArchiveAcknowledgment.VerificationPublicationArchiveEnvelopeReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationArchiveEnvelopeReference
	manifest.ArchiveAcknowledgment.VerificationPublicationArchivePackageReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationArchivePackageReference
	manifest.ArchiveAcknowledgment.VerificationPublicationArchiveIndexReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationArchiveIndexReference
	manifest.ArchiveAcknowledgment.VerificationPublicationAcknowledgmentReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationAcknowledgmentReference
	manifest.ArchiveAcknowledgment.VerificationPublicationHandoffReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationHandoffReference
	manifest.ArchiveAcknowledgment.VerificationPublicationEnvelopeReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationEnvelopeReference
	manifest.ArchiveAcknowledgment.VerificationPublicationIndexReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationIndexReference
	manifest.ArchiveAcknowledgment.VerificationPublicationReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPublicationReference
	manifest.ArchiveAcknowledgment.VerificationPackageReference = publicationArchiveHandoff.ArchiveHandoff.VerificationPackageReference
	manifest.ArchiveAcknowledgment.AttestationIndexReference = publicationArchiveHandoff.ArchiveHandoff.AttestationIndexReference
	manifest.ArchiveAcknowledgment.AttestationReference = publicationArchiveHandoff.ArchiveHandoff.AttestationReference
	manifest.ArchiveAcknowledgment.VerificationReference = publicationArchiveHandoff.ArchiveHandoff.VerificationReference
	manifest.ArchiveAcknowledgment.OperatorReference = publicationArchiveHandoff.ArchiveHandoff.OperatorReference
	manifest.ArchiveAcknowledgment.VerificationTimestamp = publicationArchiveHandoff.ArchiveHandoff.VerificationTimestamp
	manifest.ArchiveAcknowledgment.PublicationChannel = publicationArchiveHandoff.ArchiveHandoff.PublicationChannel
	manifest.ArchiveAcknowledgment.PublicationLocationReference = publicationArchiveHandoff.ArchiveHandoff.PublicationLocationReference
	manifest.ArchiveAcknowledgment.DeliveryDestinationReference = publicationArchiveHandoff.ArchiveHandoff.DeliveryDestinationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveHandoff", Digest: publicationArchiveHandoffDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveEnvelope", Digest: publicationArchiveEnvelopeDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchivePackage", Digest: publicationArchivePackageDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveIndex", Digest: publicationArchiveIndexDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationAcknowledgment", Digest: publicationAcknowledgmentDigest},
		...
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationArchiveAcknowledgmentReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.ArchiveAcknowledgment.VerificationPublicationArchiveAcknowledgmentReference,
				manifest.ArchiveAcknowledgment.AcknowledgedByReference,
				manifest.ArchiveAcknowledgment.AcknowledgmentTimestamp,
				manifest.ArchiveAcknowledgment.VerificationPublicationArchiveHandoffReference,
				manifest.ArchiveAcknowledgment.VerificationPublicationArchiveEnvelopeReference,
				manifest.ArchiveAcknowledgment.VerificationPublicationArchivePackageReference,
				manifest.ArchiveAcknowledgment.VerificationPublicationArchiveIndexReference,
				manifest.ArchiveAcknowledgment.VerificationPublicationAcknowledgmentReference,
				manifest.ArchiveAcknowledgment.VerificationPublicationHandoffReference,
				manifest.ArchiveAcknowledgment.VerificationPublicationEnvelopeReference,
				manifest.ArchiveAcknowledgment.VerificationPublicationIndexReference,
				manifest.ArchiveAcknowledgment.VerificationPublicationReference,
				manifest.ArchiveAcknowledgment.VerificationPackageReference,
				manifest.ArchiveAcknowledgment.AttestationIndexReference,
				manifest.ArchiveAcknowledgment.AttestationReference,
				manifest.ArchiveAcknowledgment.VerificationReference,
				manifest.ArchiveAcknowledgment.Continuity.AuthorizationID,
				manifest.ArchiveAcknowledgment.Continuity.TargetDigest,
				manifest.ArchiveAcknowledgment.BlockerCode,
			}, "|"))),
		},
	}
	return manifest, subjects, nil
...
func buildWorkflowRolloutClosureVerifyPublicationArchiveAttestationManifest(workspacePath string, payload workflowRolloutClosureVerifyPublicationArchiveAttestationExportRequest) (workflowRolloutClosureVerifyPublicationArchiveAttestationManifest, []audit.Subject, error) {
	publicationArchiveAcknowledgment, publicationArchiveAcknowledgmentPath, publicationArchiveAcknowledgmentDigest, err := loadLatestRolloutClosureVerifyPublicationArchiveAcknowledgment(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	}
	publicationArchiveHandoff, publicationArchiveHandoffPath, publicationArchiveHandoffDigest, err := loadLatestRolloutClosureVerifyPublicationArchiveHandoff(workspacePath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: err.Error()}
	...
	verifyMarkdownBytes, err := os.ReadFile(verifyMarkdownPath)
	if err != nil {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAT-003: latest closure verification markdown export is missing or unreadable",
		}
	}
	if !verifyExport.Export.Ready && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAT-004: latest closure verification export is blocked without archived blocked reason reference",
		}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.ArchiveAcknowledgmentState == "archive-acknowledgment-blocked" && strings.TrimSpace(verifyExport.Export.AllowBlockedReasonReference) == "" {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAT-005: latest verification publication archive acknowledgment is blocked without archived blocked reason reference",
		}
	}
	currentVerification := verifyWorkflowRolloutClosureChain(workspacePath)
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationArchiveHandoff.ArchiveHandoff.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationArchiveEnvelope.ArchiveEnvelope.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationArchivePackage.ArchivePackage.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationArchiveIndex.ArchiveIndex.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationAcknowledgment.Acknowledgment.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationHandoff.Handoff.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationEnvelope.Envelope.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationIndex.Index.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationAttestation.Publication.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != publicationPackage.Package.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != attestationIndex.Index.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != attestation.Attestation.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != verifyExport.Export.Continuity ||
		publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity != currentVerification.Verification.Continuity {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{
			Status: http.StatusUnprocessableEntity,
			Err:    "YARA-RCVPAT-006: latest publication-archive-acknowledgment/archive-handoff/archive-envelope/archive-package/archive-index/acknowledgment/handoff/envelope/index/attestation/package/attestation-index/attestation/export continuity diverges from current closure verification chain",
		}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationArchiveHandoff.Digest != publicationArchiveHandoffDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-007: latest verification publication archive acknowledgment does not bind the latest publication archive handoff digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationArchiveEnvelope.Digest != publicationArchiveEnvelopeDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-008: latest verification publication archive acknowledgment does not bind the latest publication archive envelope digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationArchivePackage.Digest != publicationArchivePackageDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-009: latest verification publication archive acknowledgment does not bind the latest publication archive package digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationArchiveIndex.Digest != publicationArchiveIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-010: latest verification publication archive acknowledgment does not bind the latest publication archive index digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationAcknowledgment.Digest != publicationAcknowledgmentDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-011: latest verification publication archive acknowledgment does not bind the latest publication acknowledgment digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationHandoff.Digest != publicationHandoffDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-012: latest verification publication archive acknowledgment does not bind the latest publication handoff digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationEnvelope.Digest != publicationEnvelopeDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-013: latest verification publication archive acknowledgment does not bind the latest publication envelope digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationIndex.Digest != publicationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-014: latest verification publication archive acknowledgment does not bind the latest publication index digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationAttestation.Digest != publicationAttestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-015: latest verification publication archive acknowledgment does not bind the latest publication attestation digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationPackage.Digest != publicationPackageDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-016: latest verification publication archive acknowledgment does not bind the latest publication package digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationAttestationIndex.Digest != attestationIndexDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-017: latest verification publication archive acknowledgment does not bind the latest attestation index digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationAttestation.Digest != attestationDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-018: latest verification publication archive acknowledgment does not bind the latest verification attestation digest"}
	}
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationExportJSON.Digest != verifyExportDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-019: latest verification publication archive acknowledgment does not bind the latest verification export json digest"}
	}
	verifyMarkdownDigest := digestBytes(verifyMarkdownBytes)
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationExportMarkdown.Digest != verifyMarkdownDigest {
		return workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{}, nil, workflowGateError{Status: http.StatusUnprocessableEntity, Err: "YARA-RCVPAT-020: latest verification publication archive acknowledgment does not bind the latest verification export markdown digest"}
	}
	archiveAttestationState := "archive-attestation-ready"
	blockerCode := ""
	if publicationArchiveAcknowledgment.ArchiveAcknowledgment.ArchiveAcknowledgmentState == "archive-acknowledgment-blocked" ||
		publicationArchiveHandoff.ArchiveHandoff.ArchiveHandoffState == "archive-handoff-blocked" ||
		publicationArchiveEnvelope.ArchiveEnvelope.ArchiveEnvelopeState == "archive-envelope-blocked" ||
		publicationArchivePackage.ArchivePackage.ArchivePackageState == "archive-package-blocked" ||
		publicationArchiveIndex.ArchiveIndex.ArchiveIndexState == "archive-index-blocked" ||
		publicationAcknowledgment.Acknowledgment.AcknowledgmentState == "acknowledgment-blocked" ||
		...
		attestation.Attestation.AttestationState == "attested-blocked" ||
		!verifyExport.Export.Ready {
		archiveAttestationState = "archive-attestation-blocked"
		blockerCode = mapValueOrDefault(publicationArchiveAcknowledgment.ArchiveAcknowledgment.BlockerCode, mapValueOrDefault(verifyExport.Export.BlockerCode, "YARA-RCVPAT-004"))
	}
	manifest := workflowRolloutClosureVerifyPublicationArchiveAttestationManifest{Valid: true}
	manifest.ArchiveAttestation.WorkspacePath = workspacePath
	manifest.ArchiveAttestation.VerificationPublicationArchiveAttestationReference = strings.TrimSpace(payload.VerificationPublicationArchiveAttestationReference)
	manifest.ArchiveAttestation.AttestedByReference = strings.TrimSpace(payload.AttestedByReference)
	manifest.ArchiveAttestation.AttestationTimestamp = strings.TrimSpace(payload.AttestationTimestamp)
	manifest.ArchiveAttestation.ArchiveAttestationState = archiveAttestationState
	manifest.ArchiveAttestation.BlockerCode = blockerCode
	manifest.ArchiveAttestation.Continuity = publicationArchiveAcknowledgment.ArchiveAcknowledgment.Continuity
	manifest.ArchiveAttestation.VerificationPublicationArchiveAcknowledgment = workflowClosureArtifact{Path: publicationArchiveAcknowledgmentPath, Digest: publicationArchiveAcknowledgmentDigest}
	manifest.ArchiveAttestation.VerificationPublicationArchiveHandoff = workflowClosureArtifact{Path: publicationArchiveHandoffPath, Digest: publicationArchiveHandoffDigest}
	manifest.ArchiveAttestation.VerificationPublicationArchiveEnvelope = workflowClosureArtifact{Path: publicationArchiveEnvelopePath, Digest: publicationArchiveEnvelopeDigest}
	manifest.ArchiveAttestation.VerificationPublicationArchivePackage = workflowClosureArtifact{Path: publicationArchivePackagePath, Digest: publicationArchivePackageDigest}
	manifest.ArchiveAttestation.VerificationPublicationArchiveIndex = workflowClosureArtifact{Path: publicationArchiveIndexPath, Digest: publicationArchiveIndexDigest}
	manifest.ArchiveAttestation.VerificationPublicationAcknowledgment = workflowClosureArtifact{Path: publicationAcknowledgmentPath, Digest: publicationAcknowledgmentDigest}
	manifest.ArchiveAttestation.VerificationPublicationHandoff = workflowClosureArtifact{Path: publicationHandoffPath, Digest: publicationHandoffDigest}
	manifest.ArchiveAttestation.VerificationPublicationEnvelope = workflowClosureArtifact{Path: publicationEnvelopePath, Digest: publicationEnvelopeDigest}
	manifest.ArchiveAttestation.VerificationPublicationIndex = workflowClosureArtifact{Path: publicationIndexPath, Digest: publicationIndexDigest}
	manifest.ArchiveAttestation.VerificationPublicationAttestation = workflowClosureArtifact{Path: publicationAttestationPath, Digest: publicationAttestationDigest}
	manifest.ArchiveAttestation.VerificationPublicationPackage = workflowClosureArtifact{Path: publicationPackagePath, Digest: publicationPackageDigest}
	manifest.ArchiveAttestation.VerificationAttestationIndex = workflowClosureArtifact{Path: attestationIndexPath, Digest: attestationIndexDigest}
	manifest.ArchiveAttestation.VerificationAttestation = workflowClosureArtifact{Path: attestationPath, Digest: attestationDigest}
	manifest.ArchiveAttestation.VerificationExportJSON = workflowClosureArtifact{Path: verifyExportPath, Digest: verifyExportDigest}
	manifest.ArchiveAttestation.VerificationExportMarkdown = workflowClosureArtifact{Path: verifyMarkdownPath, Digest: verifyMarkdownDigest}
	manifest.ArchiveAttestation.VerificationPublicationArchiveAcknowledgmentReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationArchiveAcknowledgmentReference
	manifest.ArchiveAttestation.VerificationPublicationArchiveHandoffReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationArchiveHandoffReference
	manifest.ArchiveAttestation.VerificationPublicationArchiveEnvelopeReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationArchiveEnvelopeReference
	manifest.ArchiveAttestation.VerificationPublicationArchivePackageReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationArchivePackageReference
	manifest.ArchiveAttestation.VerificationPublicationArchiveIndexReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationArchiveIndexReference
	manifest.ArchiveAttestation.VerificationPublicationAcknowledgmentReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationAcknowledgmentReference
	manifest.ArchiveAttestation.VerificationPublicationHandoffReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationHandoffReference
	manifest.ArchiveAttestation.VerificationPublicationEnvelopeReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationEnvelopeReference
	manifest.ArchiveAttestation.VerificationPublicationIndexReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationIndexReference
	manifest.ArchiveAttestation.VerificationPublicationReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPublicationReference
	manifest.ArchiveAttestation.VerificationPackageReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationPackageReference
	manifest.ArchiveAttestation.AttestationIndexReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.AttestationIndexReference
	manifest.ArchiveAttestation.AttestationReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.AttestationReference
	manifest.ArchiveAttestation.VerificationReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationReference
	manifest.ArchiveAttestation.OperatorReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.OperatorReference
	manifest.ArchiveAttestation.VerificationTimestamp = publicationArchiveAcknowledgment.ArchiveAcknowledgment.VerificationTimestamp
	manifest.ArchiveAttestation.PublicationChannel = publicationArchiveAcknowledgment.ArchiveAcknowledgment.PublicationChannel
	manifest.ArchiveAttestation.PublicationLocationReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.PublicationLocationReference
	manifest.ArchiveAttestation.DeliveryDestinationReference = publicationArchiveAcknowledgment.ArchiveAcknowledgment.DeliveryDestinationReference
	subjects := []audit.Subject{
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveAcknowledgment", Digest: publicationArchiveAcknowledgmentDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveHandoff", Digest: publicationArchiveHandoffDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveEnvelope", Digest: publicationArchiveEnvelopeDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchivePackage", Digest: publicationArchivePackageDigest},
		{Kind: "WorkflowRolloutClosureVerifyPublicationArchiveIndex", Digest: publicationArchiveIndexDigest},
		...
		{Kind: "WorkflowRolloutClosureVerifyMarkdown", Digest: verifyMarkdownDigest},
		{
			Kind: "WorkflowRolloutClosureVerifyPublicationArchiveAttestationReference",
			Digest: digestBytes([]byte(strings.Join([]string{
				manifest.ArchiveAttestation.VerificationPublicationArchiveAttestationReference,
				manifest.ArchiveAttestation.AttestedByReference,
				manifest.ArchiveAttestation.AttestationTimestamp,
				manifest.ArchiveAttestation.VerificationPublicationArchiveAcknowledgmentReference,
				manifest.ArchiveAttestation.VerificationPublicationArchiveHandoffReference,
				manifest.ArchiveAttestation.VerificationPublicationArchiveEnvelopeReference,
				manifest.ArchiveAttestation.VerificationPublicationArchivePackageReference,
				manifest.ArchiveAttestation.VerificationPublicationArchiveIndexReference,
				manifest.ArchiveAttestation.VerificationPublicationAcknowledgmentReference,
				manifest.ArchiveAttestation.VerificationPublicationHandoffReference,
				manifest.ArchiveAttestation.VerificationPublicationEnvelopeReference,
				manifest.ArchiveAttestation.VerificationPublicationIndexReference,
				manifest.ArchiveAttestation.VerificationPublicationReference,
				manifest.ArchiveAttestation.VerificationPackageReference,
				manifest.ArchiveAttestation.AttestationIndexReference,
				manifest.ArchiveAttestation.AttestationReference,
				manifest.ArchiveAttestation.VerificationReference,
				manifest.ArchiveAttestation.Continuity.AuthorizationID,
				manifest.ArchiveAttestation.Continuity.TargetDigest,
				manifest.ArchiveAttestation.BlockerCode,
			}, "|"))),
		},
	}
	return manifest, subjects, nil
...
func discoverRolloutClosureVerifyPublicationAttestationExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-attestation.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
...
func discoverRolloutClosureVerifyPublicationIndexExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-index.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
...
func discoverRolloutClosureVerifyPublicationEnvelopeExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-envelope.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
...
func discoverRolloutClosureVerifyPublicationHandoffExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-handoff.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
...
func discoverRolloutClosureVerifyPublicationAcknowledgmentExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-acknowledgment.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
...
func discoverRolloutClosureVerifyPublicationArchiveIndexExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-archive-index.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
...
func discoverRolloutClosureVerifyPublicationArchivePackageExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-archive-package.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
...
func discoverRolloutClosureVerifyPublicationArchiveEnvelopeExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-archive-envelope.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
...
func discoverRolloutClosureVerifyPublicationArchiveHandoffExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-archive-handoff.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
...
func discoverRolloutClosureVerifyPublicationArchiveAcknowledgmentExports(workspacePath string) []string {
	entries, err := os.ReadDir(workspacePath)
	if err != nil {
		return []string{}
	}
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if strings.HasSuffix(name, ".rollout-closure-verify.publication-archive-acknowledgment.json") {
			paths = append(paths, filepath.Join(workspacePath, name))
		}
	}
	sort.Strings(paths)
