...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/attest/index/export` now persists deterministic attestation-index manifests + mandatory audit output that bind latest verification attestation/export markdown/json digests, continuity IDs, and explicit publication metadata into an immutable registry record;
  - attestation-index export now requires explicit `attestationIndexReference` + `publishedByReference` + `publishedTimestamp` and fails closed when latest verification attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVAI-*`);
  - UI capsule panel now supports verification-attestation-index export with explicit index pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I35 — Closure-chain verification, export bundle, digest attestation, and attestation index
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, and `POST /api/v1/workflow/rollout-closure/verify/attest/index/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`). Status: completed.
## Next implementation slice
Implement **I36 — Closure-chain verification publication package export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-package/export` to persist one deterministic publication package that binds the latest attestation-index manifest digest with verification attestation/export markdown/json digests, continuity IDs, and explicit package metadata for reviewer handoff;
- require explicit `verificationPackageReference` + `packagedByReference` + `packagedTimestamp` and fail closed when latest attestation-index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVP-*`) for verification-publication-package precondition/path failures;
- extend capsule UI with verification-publication-package export action and explicit pass/blocked diagnostics for package handoff readiness.
Acceptance criteria:
- verification publication package export persists deterministic package artifacts bound to latest attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-package flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
