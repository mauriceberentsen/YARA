...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export` now persists deterministic verification publication-attestation manifests + mandatory audit output that bind latest verification publication-package + attestation-index + attestation + export markdown/json digests, continuity IDs, and explicit publication-channel metadata;
  - publication-attestation export now requires explicit `verificationPublicationReference` + `publishedByReference` + `publishedTimestamp` (+ channel/location metadata) and fails closed when latest publication-package/attestation-index/attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVPA-*`);
  - UI capsule panel now supports verification-publication-attestation export with explicit publication pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I37 — Closure-chain verification, export bundle, digest attestation, attestation index, publication package, and publication attestation
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, `POST /api/v1/workflow/rollout-closure/verify/attest/index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-package/export`, and `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index/publication-package/publication-attestation exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`, `YARA-RCVP-*`, `YARA-RCVPA-*`). Status: completed.
## Next implementation slice
Implement **I38 — Closure-chain verification publication index export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-index/export` to persist one deterministic verification publication-index manifest that binds latest publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests into a single operator index record;
- require explicit `verificationPublicationIndexReference` + `indexedByReference` + `indexedTimestamp` and fail closed when latest publication-attestation/package/attestation-index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVPI-*`) for verification-publication-index precondition/path failures;
- extend capsule UI with verification-publication-index export action and explicit pass/blocked diagnostics for index readiness.
Acceptance criteria:
- verification publication index export persists deterministic index artifacts bound to latest publication-attestation + publication-package + attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-index flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
