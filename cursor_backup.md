...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/publication-archive-index/export` now persists deterministic verification publication-archive-index manifests + mandatory audit output that bind latest verification publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit archive-index metadata;
  - publication-archive-index export now requires explicit `verificationPublicationArchiveIndexReference` + `indexedByReference` + `indexedTimestamp` and fails closed when latest publication-acknowledgment/handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVPAX-*`);
  - UI capsule panel now supports verification-publication-archive-index export with explicit archive-index pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I42 — Closure-chain verification, export bundle, digest attestation, attestation index, publication package, publication attestation, publication index, publication envelope, publication handoff, publication acknowledgment, and publication archive index
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, `POST /api/v1/workflow/rollout-closure/verify/attest/index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-package/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-envelope/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-handoff/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-acknowledgment/export`, and `POST /api/v1/workflow/rollout-closure/verify/publication-archive-index/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index/publication-package/publication-attestation/publication-index/publication-envelope/publication-handoff/publication-acknowledgment/publication-archive-index exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`, `YARA-RCVP-*`, `YARA-RCVPA-*`, `YARA-RCVPI-*`, `YARA-RCVPE-*`, `YARA-RCVPH-*`, `YARA-RCVPAK-*`, `YARA-RCVPAX-*`). Status: completed.
## Next implementation slice
Implement **I43 — Closure-chain verification publication archive package export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-archive-package/export` to persist one deterministic verification publication-archive-package manifest that binds latest publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit archive-package metadata;
- require explicit `verificationPublicationArchivePackageReference` + `packagedByReference` + `packagedTimestamp` and fail closed when latest publication-archive-index/acknowledgment/handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVPAP-*`) for verification-publication-archive-package precondition/path failures;
- extend capsule UI with verification-publication-archive-package export action and explicit pass/blocked diagnostics for archive-package readiness.
Acceptance criteria:
- verification publication archive-package export persists deterministic artifacts bound to latest publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-archive-package flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
