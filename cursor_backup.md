...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/publication-index/export` now persists deterministic verification publication-index manifests + mandatory audit output that bind latest verification publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests into one continuity-preserving operator index record;
  - publication-index export now requires explicit `verificationPublicationIndexReference` + `indexedByReference` + `indexedTimestamp` and fails closed when latest publication-attestation/package/attestation-index/attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVPI-*`);
  - UI capsule panel now supports verification-publication-index export with explicit index pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I38 — Closure-chain verification, export bundle, digest attestation, attestation index, publication package, publication attestation, and publication index
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, `POST /api/v1/workflow/rollout-closure/verify/attest/index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-package/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export`, and `POST /api/v1/workflow/rollout-closure/verify/publication-index/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index/publication-package/publication-attestation/publication-index exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`, `YARA-RCVP-*`, `YARA-RCVPA-*`, `YARA-RCVPI-*`). Status: completed.
## Next implementation slice
Implement **I39 — Closure-chain verification publication delivery envelope export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-envelope/export` to persist one deterministic verification publication-envelope manifest that binds latest publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit delivery destination metadata;
- require explicit `verificationPublicationEnvelopeReference` + `deliveredByReference` + `deliveryTimestamp` + `deliveryDestinationReference` and fail closed when latest publication-index/attestation/package/index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVPE-*`) for verification-publication-envelope precondition/path failures;
- extend capsule UI with verification-publication-envelope export action and explicit pass/blocked diagnostics for delivery readiness.
Acceptance criteria:
- verification publication envelope export persists deterministic envelope artifacts bound to latest publication-index + publication-attestation + publication-package + attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-envelope flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
