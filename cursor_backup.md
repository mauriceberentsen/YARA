...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/publication-package/export` now persists deterministic verification publication-package manifests + mandatory audit output that bind latest verification attestation-index + attestation + export markdown/json digests, continuity IDs, and explicit package metadata for reviewer handoff;
  - publication-package export now requires explicit `verificationPackageReference` + `packagedByReference` + `packagedTimestamp` and fails closed when latest attestation-index/attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVP-*`);
  - UI capsule panel now supports verification-publication-package export with explicit package pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I36 — Closure-chain verification, export bundle, digest attestation, attestation index, and publication package
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, `POST /api/v1/workflow/rollout-closure/verify/attest/index/export`, and `POST /api/v1/workflow/rollout-closure/verify/publication-package/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index/publication-package exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`, `YARA-RCVP-*`). Status: completed.
## Next implementation slice
Implement **I37 — Closure-chain verification publication attestation export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export` to persist one deterministic publication attestation that binds the latest verification publication-package digest with attestation-index/attestation/export markdown/json digests, continuity IDs, and explicit publication-channel metadata;
- require explicit `verificationPublicationReference` + `publishedByReference` + `publishedTimestamp` and fail closed when latest publication-package/attestation-index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVPA-*`) for verification-publication-attestation precondition/path failures;
- extend capsule UI with verification-publication-attestation export action and explicit pass/blocked diagnostics for publication readiness.
Acceptance criteria:
- verification publication attestation export persists deterministic attestation artifacts bound to latest publication-package + attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-attestation flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
