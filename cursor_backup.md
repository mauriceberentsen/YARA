...
- Recent commits (newest first): `cfaa249`, `3eceb79`, `e6f3c28`, `90dc9a6`, `ad25b24`.
...
- Interactive workflow cockpit I17-I27 are implemented:
  - `POST /api/v1/workflow/release-publication/export`, `.../index/export`, `.../package/export`, `.../envelope/export`, `.../handoff-receipt/export`, `.../acknowledgment/export`, `POST /api/v1/workflow/rollout-closure-summary/export`, `.../rollout-closure-delivery/export`, `.../rollout-closure-acceptance/export`, `.../rollout-closure-certificate/export`, and `.../rollout-closure-ledger/export` now persist deterministic publication-chain + closure manifests bound to capsule/evidence/closure/review/decision/publication digests;
  - exports fail closed on missing/blocked chain artifacts, malformed publication metadata, and continuity/digest divergence (`YARA-RPB-*`, `YARA-RPI-*`, `YARA-RPK-*`, `YARA-RPE-*`, `YARA-RHR-*`, `YARA-RAK-*`, `YARA-RCS-*`, `YARA-RCD-*`, `YARA-RCA-*`, `YARA-RCC-*`, `YARA-RLG-*`) with workspace-bounded no-overwrite audit outputs;
  - capsule UI now supports publication attestation/index/package/envelope/handoff/acknowledgment/closure-summary/delivery-record/acceptance/certificate/ledger export and surfaces explicit `publishable`, `index-ready`, `package-ready`, `delivery-ready`, `handoff-ready`, `acknowledgment-ready`, `summary-ready`, `delivery-record-ready`, `acceptance-ready`, `certificate-ready`, and `ledger-ready` diagnostics.
...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure-ledger/export` now writes deterministic rollout closure archival-ledger manifests + mandatory audit output with workspace-bounded no-overwrite semantics;
  - ledger export now requires explicit `ledgerReference` + `recordedByReference` + `recordedTimestamp` and fails closed on missing/blocked certificate/publication-chain artifacts or continuity/digest divergence;
  - UI capsule panel now supports ledger export and surfaces artifact paths plus explicit `ledger-ready` / `blocked` diagnostics.
...
### I25-I27 — Rollout closure acceptance receipt + certificate + archival ledger export
- add `POST /api/v1/workflow/rollout-closure-acceptance/export`, `.../rollout-closure-certificate/export`, and `.../rollout-closure-ledger/export` to persist deterministic acceptance/certificate/ledger manifests that bind delivery/summary/acknowledgment/handoff/envelope/package/index/attestation/decision/closure/review digests with explicit `acceptanceReference`/`acceptedByReference`/`acceptanceTimestamp`, `certificateReference`/`issuedByReference`/`issuedTimestamp`, and `ledgerReference`/`recordedByReference`/`recordedTimestamp`; fail closed on missing/blocked artifacts, malformed metadata, or continuity/digest divergence (`YARA-RCA-*`, `YARA-RCC-*`, `YARA-RLG-*`) with mandatory workspace-bounded no-overwrite audit output. Status: completed.
## Next implementation slice
Implement **I28 — Rollout closure handoff docket export**:
- add `POST /api/v1/workflow/rollout-closure-docket/export` to persist one deterministic handoff-docket manifest that binds ledger, certificate, acceptance-receipt, delivery-record, closure-summary, acknowledgment, handoff, envelope, package, index, attestation, decision, closure, and review digests as the final transfer-ready bundle reference;
- require explicit `docketReference` + `preparedByReference` + `preparedTimestamp`; fail closed when any linked artifact is missing, malformed, blocked, or continuity diverges;
- emit mandatory audit output with workspace-bounded no-overwrite semantics and deterministic blocker codes for docket export failures;
- extend capsule UI with rollout closure docket export action and explicit "docket ready / blocked" diagnostics.
Acceptance criteria:
- rollout closure docket export writes deterministic handoff-docket manifest + audit artifacts bound to closure/review/decision/publication/index/package/envelope/handoff/acknowledgment/summary/delivery/acceptance/certificate/ledger continuity digests;
- rollout closure docket export fails closed on missing/blocked/malformed publication artifacts and out-of-workspace or duplicate output paths;
- UI rollout closure docket export flow surfaces artifact paths and fail-closed diagnostics without exposing secret-bearing fields;
