...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/publication-acknowledgment/export` now persists deterministic verification publication-acknowledgment manifests + mandatory audit output that bind latest verification publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit acknowledgment metadata;
  - publication-acknowledgment export now requires explicit `verificationPublicationAcknowledgmentReference` + `acknowledgedByReference` + `acknowledgmentTimestamp` and fails closed when latest publication-handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVPAK-*`);
  - UI capsule panel now supports verification-publication-acknowledgment export with explicit acknowledgment pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I41 — Closure-chain verification, export bundle, digest attestation, attestation index, publication package, publication attestation, publication index, publication envelope, publication handoff, and publication acknowledgment
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, `POST /api/v1/workflow/rollout-closure/verify/attest/index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-package/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-envelope/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-handoff/export`, and `POST /api/v1/workflow/rollout-closure/verify/publication-acknowledgment/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index/publication-package/publication-attestation/publication-index/publication-envelope/publication-handoff/publication-acknowledgment exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`, `YARA-RCVP-*`, `YARA-RCVPA-*`, `YARA-RCVPI-*`, `YARA-RCVPE-*`, `YARA-RCVPH-*`, `YARA-RCVPAK-*`). Status: completed.
## Next implementation slice
Implement **I42 — Closure-chain verification publication archive index export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-archive-index/export` to persist one deterministic verification publication-archive-index manifest that binds latest publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit archive-index metadata;
- require explicit `verificationPublicationArchiveIndexReference` + `indexedByReference` + `indexedTimestamp` and fail closed when latest publication-acknowledgment/handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVPAX-*`) for verification-publication-archive-index precondition/path failures;
- extend capsule UI with verification-publication-archive-index export action and explicit pass/blocked diagnostics for archive-index readiness.
Acceptance criteria:
- verification publication archive-index export persists deterministic artifacts bound to latest publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-archive-index flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
