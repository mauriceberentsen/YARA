...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/publication-archive-handoff/export` now persists deterministic verification publication-archive-handoff manifests + mandatory audit output that bind latest verification publication-archive-envelope + publication-archive-package + publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit archive-handoff metadata;
  - publication-archive-handoff export now requires explicit `verificationPublicationArchiveHandoffReference` + `receivedByReference` + `handoffTimestamp` and fails closed when latest publication-archive-envelope/archive-package/archive-index/acknowledgment/handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVPAH-*`);
  - UI capsule panel now supports verification-publication-archive-handoff export with explicit archive-handoff pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I45 — Closure-chain verification, export bundle, digest attestation, attestation index, publication package, publication attestation, publication index, publication envelope, publication handoff, publication acknowledgment, publication archive index, publication archive package, publication archive envelope, and publication archive handoff
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, `POST /api/v1/workflow/rollout-closure/verify/attest/index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-package/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-envelope/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-handoff/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-acknowledgment/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-archive-index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-archive-package/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-archive-envelope/export`, and `POST /api/v1/workflow/rollout-closure/verify/publication-archive-handoff/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index/publication-package/publication-attestation/publication-index/publication-envelope/publication-handoff/publication-acknowledgment/publication-archive-index/publication-archive-package/publication-archive-envelope/publication-archive-handoff exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`, `YARA-RCVP-*`, `YARA-RCVPA-*`, `YARA-RCVPI-*`, `YARA-RCVPE-*`, `YARA-RCVPH-*`, `YARA-RCVPAK-*`, `YARA-RCVPAX-*`, `YARA-RCVPAP-*`, `YARA-RCVPAE-*`, `YARA-RCVPAH-*`). Status: completed.
## Next implementation slice
Implement **I46 — Closure-chain verification publication archive acknowledgment export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-archive-acknowledgment/export` to persist one deterministic verification publication-archive-acknowledgment manifest that binds latest publication-archive-handoff + publication-archive-envelope + publication-archive-package + publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit archive-acknowledgment metadata;
- require explicit `verificationPublicationArchiveAcknowledgmentReference` + `acknowledgedByReference` + `acknowledgmentTimestamp` and fail closed when latest publication-archive-handoff/archive-envelope/archive-package/archive-index/acknowledgment/handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVPAA-*`) for verification-publication-archive-acknowledgment precondition/path failures;
- extend capsule UI with verification-publication-archive-acknowledgment export action and explicit pass/blocked diagnostics for archive-acknowledgment readiness.
Acceptance criteria:
- verification publication archive-acknowledgment export persists deterministic artifacts bound to latest publication-archive-handoff + publication-archive-envelope + publication-archive-package + publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-archive-acknowledgment flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
