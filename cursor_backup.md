...
## Controlled review: disposable NVIDIA Kubernetes environment

On 2026-07-20 a disposable, single-node minikube profile was provisioned over Docker on the authorized remote GB10 host. It runs Kubernetes v1.35.6 on `linux/arm64`, Calico and the NVIDIA device plugin. The node reports one allocatable `nvidia.com/gpu`; an isolated CUDA vector-add Pod completed with `Test PASSED`. Existing host containers and data were left untouched.

The cross-compiled Linux/ARM64 YARA binary validated catalog v0.2, the Kubernetes bundle and all ten v0.1 golden scenarios on the remote host. Their audit chains verified. After provisioning the exact YARA-owned namespace, a bound disposable model PVC and the pinned model snapshot, the real read-only preflight passed Kubernetes version, API discovery, DNS, GPU capacity, node-platform compatibility, namespace ownership and PVC binding. Only the four intended active-verification checks remained blocked. The real change set contained 11 creates, one namespace no-op, zero conflicts and zero deletes.

The first authorized apply found a real portability defect before bundle mutation: the prerequisite Pod used `python`, while the pinned ARM64 vLLM image exposes `/usr/bin/python3`. The branch `fix/kubernetes-verifier-python3` now uses the absolute interpreter path for both verifier Pods, asserts it in executor tests and polls Pod status so terminal startup failures stop immediately instead of waiting the full 15-minute bound. A manually equivalent hardened Pod proved `/usr/bin/python3`, the read-only PVC and executable `emptyDir` together on the target. The valid fail-closed partial receipt is `sha256:0c13c460e615b2ee03c48aaee49b51db74ab433d1739eafb556b8536f4706e76`; its terminal audit head is `sha256:75311938cf451b947d5dd3614cc660f610f0ae5ff28e3a4b6821d0306b2dc36c`. No bundle object was applied in that run.

The corrected verifier exposed two further live-cluster gaps. Kubernetes 1.35 injects probe `successThreshold`/HTTP scheme and ConfigMap `defaultMode` defaults; these are now removed only when they equal the exact server defaults, preserving drift visibility for non-default values. The subsequent workload start also proved that the pinned vLLM/FlashInfer image needs writable runtime caches. The renderer retains `readOnlyRootFilesystem: true` and redirects `HOME`, XDG cache/config and `FLASHINFER_WORKSPACE_BASE` to the existing `/tmp` `emptyDir`. Unit tests cover both the server-default projection and rendered cache contract.

The newly rendered bundle `sha256:f58aaf237201b64e50f50db9c5ca74b028a5a688c748c08ffdd729443b6862a9` was then applied successfully. YARA actively verified model digests, executable `/tmp`, both Deployment rollouts, bounded inference through LiteLLM and denial of direct verifier-to-vLLM traffic under Calico. The valid success receipt is `sha256:e584d749052c4b389e9013745337d76ccf02862d5fda900eec6c90c8d634944f`; its audit head is `sha256:2cd9937fd16d900ee2b2959e6f77cce2e9937686a5b61c13362392ac9ec5902e`.

A fresh independently reviewed repeat produced a change set with exactly 12 no-ops and zero creates, updates, conflicts or deletes. Its apply also succeeded, with all 12 receipt operations `unchanged` and no `applied` operation. The idempotency receipt is `sha256:caa1d717287be833152da68101dc61a52ad0bac54509132413e93adab79c7e7d`; its audit head is `sha256:946da1bf551404f57781961281ef94d9e6bf97f9bad0624fc5073549577fcd8a`. Both workloads remain Ready for inspection in the disposable profile. Temporary signing private keys and the redundant host-side model download were deleted; no verifier Pod or Lease remains.
