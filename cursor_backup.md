...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/publication-archive-envelope/export` now persists deterministic verification publication-archive-envelope manifests + mandatory audit output that bind latest verification publication-archive-package + publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit archive-envelope metadata;
  - publication-archive-envelope export now requires explicit `verificationPublicationArchiveEnvelopeReference` + `deliveredByReference` + `deliveryTimestamp` + `deliveryDestinationReference` and fails closed when latest publication-archive-package/archive-index/acknowledgment/handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVPAE-*`);
  - UI capsule panel now supports verification-publication-archive-envelope export with explicit archive-envelope pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I44 — Closure-chain verification, export bundle, digest attestation, attestation index, publication package, publication attestation, publication index, publication envelope, publication handoff, publication acknowledgment, publication archive index, publication archive package, and publication archive envelope
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, `POST /api/v1/workflow/rollout-closure/verify/attest/index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-package/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-envelope/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-handoff/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-acknowledgment/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-archive-index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-archive-package/export`, and `POST /api/v1/workflow/rollout-closure/verify/publication-archive-envelope/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index/publication-package/publication-attestation/publication-index/publication-envelope/publication-handoff/publication-acknowledgment/publication-archive-index/publication-archive-package/publication-archive-envelope exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`, `YARA-RCVP-*`, `YARA-RCVPA-*`, `YARA-RCVPI-*`, `YARA-RCVPE-*`, `YARA-RCVPH-*`, `YARA-RCVPAK-*`, `YARA-RCVPAX-*`, `YARA-RCVPAP-*`, `YARA-RCVPAE-*`). Status: completed.
## Next implementation slice
Implement **I45 — Closure-chain verification publication archive handoff receipt export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-archive-handoff/export` to persist one deterministic verification publication-archive-handoff manifest that binds latest publication-archive-envelope + publication-archive-package + publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit archive-handoff metadata;
- require explicit `verificationPublicationArchiveHandoffReference` + `receivedByReference` + `handoffTimestamp` and fail closed when latest publication-archive-envelope/archive-package/archive-index/acknowledgment/handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVPAH-*`) for verification-publication-archive-handoff precondition/path failures;
- extend capsule UI with verification-publication-archive-handoff export action and explicit pass/blocked diagnostics for archive-handoff readiness.
Acceptance criteria:
- verification publication archive-handoff export persists deterministic artifacts bound to latest publication-archive-envelope + publication-archive-package + publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-archive-handoff flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
