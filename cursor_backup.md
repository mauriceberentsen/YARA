...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/publication-archive-package/export` now persists deterministic verification publication-archive-package manifests + mandatory audit output that bind latest verification publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit archive-package metadata;
  - publication-archive-package export now requires explicit `verificationPublicationArchivePackageReference` + `packagedByReference` + `packagedTimestamp` and fails closed when latest publication-archive-index/acknowledgment/handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVPAP-*`);
  - UI capsule panel now supports verification-publication-archive-package export with explicit archive-package pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I43 — Closure-chain verification, export bundle, digest attestation, attestation index, publication package, publication attestation, publication index, publication envelope, publication handoff, publication acknowledgment, publication archive index, and publication archive package
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, `POST /api/v1/workflow/rollout-closure/verify/attest/index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-package/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-envelope/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-handoff/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-acknowledgment/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-archive-index/export`, and `POST /api/v1/workflow/rollout-closure/verify/publication-archive-package/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index/publication-package/publication-attestation/publication-index/publication-envelope/publication-handoff/publication-acknowledgment/publication-archive-index/publication-archive-package exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`, `YARA-RCVP-*`, `YARA-RCVPA-*`, `YARA-RCVPI-*`, `YARA-RCVPE-*`, `YARA-RCVPH-*`, `YARA-RCVPAK-*`, `YARA-RCVPAX-*`, `YARA-RCVPAP-*`). Status: completed.
## Next implementation slice
Implement **I44 — Closure-chain verification publication archive envelope export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-archive-envelope/export` to persist one deterministic verification publication-archive-envelope manifest that binds latest publication-archive-package + publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit archive-envelope metadata;
- require explicit `verificationPublicationArchiveEnvelopeReference` + `deliveredByReference` + `deliveryTimestamp` + `deliveryDestinationReference` and fail closed when latest publication-archive-package/archive-index/acknowledgment/handoff/envelope/index/attestation/package/index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVPAE-*`) for verification-publication-archive-envelope precondition/path failures;
- extend capsule UI with verification-publication-archive-envelope export action and explicit pass/blocked diagnostics for archive-envelope readiness.
Acceptance criteria:
- verification publication archive-envelope export persists deterministic artifacts bound to latest publication-archive-package + publication-archive-index + publication-acknowledgment + publication-handoff + publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-archive-envelope flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
