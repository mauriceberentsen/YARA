...
- This slice completed:
  - `POST /api/v1/workflow/rollout-closure/verify/publication-envelope/export` now persists deterministic verification publication-envelope manifests + mandatory audit output that bind latest verification publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit delivery destination metadata;
  - publication-envelope export now requires explicit `verificationPublicationEnvelopeReference` + `deliveredByReference` + `deliveryTimestamp` + `deliveryDestinationReference` and fails closed when latest publication-index/attestation/package/index/attestation/export artifacts are missing/malformed, blocked without archived reason, or continuity-divergent (`YARA-RCVPE-*`);
  - UI capsule panel now supports verification-publication-envelope export with explicit envelope pass/blocked diagnostics and deterministic manifest/audit path reporting without exposing secret-bearing fields.
...
### I32-I39 — Closure-chain verification, export bundle, digest attestation, attestation index, publication package, publication attestation, publication index, and publication envelope
- add `GET /api/v1/workflow/rollout-closure/verify`, `POST /api/v1/workflow/rollout-closure/verify/export`, `POST /api/v1/workflow/rollout-closure/verify/attest`, `POST /api/v1/workflow/rollout-closure/verify/attest/index/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-package/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-attestation/export`, `POST /api/v1/workflow/rollout-closure/verify/publication-index/export`, and `POST /api/v1/workflow/rollout-closure/verify/publication-envelope/export` to provide deterministic read-only closure-chain verification plus deterministic verification bundle/attestation/attestation-index/publication-package/publication-attestation/publication-index/publication-envelope exports with explicit blocked archival requirements and blocker taxonomies (`YARA-RCV-*`, `YARA-RCVX-*`, `YARA-RCVA-*`, `YARA-RCVAI-*`, `YARA-RCVP-*`, `YARA-RCVPA-*`, `YARA-RCVPI-*`, `YARA-RCVPE-*`). Status: completed.
## Next implementation slice
Implement **I40 — Closure-chain verification publication handoff receipt export**:
- add `POST /api/v1/workflow/rollout-closure/verify/publication-handoff/export` to persist one deterministic verification publication-handoff manifest that binds latest publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + attestation + export markdown/json digests with explicit receiver confirmation metadata;
- require explicit `verificationPublicationHandoffReference` + `receivedByReference` + `handoffTimestamp` and fail closed when latest publication-envelope/index/attestation/package/index/attestation/export artifacts are missing, malformed, blocked without archived reason, or continuity diverges;
- enforce workspace-bounded no-overwrite output semantics and deterministic blocker taxonomy (`YARA-RCVPH-*`) for verification-publication-handoff precondition/path failures;
- extend capsule UI with verification-publication-handoff export action and explicit pass/blocked diagnostics for handoff readiness.
Acceptance criteria:
- verification publication handoff export persists deterministic handoff artifacts bound to latest publication-envelope + publication-index + publication-attestation + publication-package + attestation-index + verification attestation/export digests and continuity metadata, and fails closed when prerequisites are missing/malformed/blocked-without-reason or output paths are out-of-workspace/duplicate;
- UI verification publication-handoff flow surfaces readiness/blocked outcomes and deterministic diagnostics without exposing secret-bearing fields;
