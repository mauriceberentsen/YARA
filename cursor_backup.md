...
- No implicit delete, prune, adoption, rollback, bootstrap provisioning, or model import path exists.
...
- Safe owned retirement and explicit rollback primitives are unimplemented.
...
Implement **safe owned-resource retirement as a separately authorized operation**:

- Add a distinct command and contract (not ordinary apply) for deleting only exact YARA-owned resources from a reviewed operation set.
- Require fresh preflight/change-set/review/signed authorization bindings tailored to retirement constraints.
- Keep deletion scope explicit and bounded; never add implicit prune/adoption.

Acceptance criteria for this slice:

- retirement fails closed when ownership, target identity, or approved operation set drifts;
- durable receipt/audit prove exactly what was removed and what was skipped/blocked;
...
- determinism: identical retirement inputs produce identical operation ordering/evidence identities.
...
- focused unit tests for the retirement contract and ownership constraints;
- negative validation tests (foreign ownership, stale state, unauthorized delete set);
