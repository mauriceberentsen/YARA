...
The example represents a local air-gapped coding/chat environment for a small team. Running `yara plan create` with the request, inventory and `catalog/v0.1/snapshot.yaml` reproduces the semantic plan. Its purpose is to prove separation of intent, facts and decisions—not to recommend a real stack.
