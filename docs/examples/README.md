...
Examples illustrate the executable v1alpha1 resource model and review experience. The original files reproduce the frozen v0.1 placeholder acceptance suite. The v0.2 request and inventory exercise the first real, still-experimental catalog path.
...
- [v0.2 request for a real chat/coding stack](v0.2-platform-request.yaml)
- [v0.2 RTX 4090 inventory](v0.2-inventory.yaml)
...
With `catalog/v0.2/snapshot.yaml`, the v0.2 pair selects LiteLLM 1.93.0, vLLM 0.25.1 and the immutable Qwen2.5 Coder 7B AWQ snapshot for the declared RTX 4090. This output remains `review-required`: exact artifacts are known, but operational compatibility has not yet passed YARA's hardware contract-test gate.
