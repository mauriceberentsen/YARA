...
The [offline renderers](rendering.md) translate the exact LiteLLM/vLLM plan into content-addressed Docker Compose or Kubernetes/GitOps `DeploymentBundle` resources. Both remain pure and never acquire or deploy anything. The [Kubernetes target preflight](target-preflight.md), [change-set and approval flow](change-sets-and-approvals.md), and short-lived signed authorization lead to the [initial direct executor](kubernetes-apply.md). A separate bounded `deployment bootstrap kubernetes` command now provisions a YARA-owned namespace plus model PVC with explicit confirmations and immutable receipt/audit evidence; apply remains a distinct command with no implicit bootstrap.
...
- Full clean-cluster installation and acquisition/import execution automation remain deferred beyond the bounded bootstrap + executor paths. Safe owned-resource retirement and rollback are separate authorized paths.
